...
export const ACCOUNTS = {
  ...
  "CASH-IN-HAND": { name: "Cash in Hand", type: "asset" },
  "BANK CURRENT ACCOUNT": { name: "Bank Current Account", type: "asset" },
  "ACCOUNTS RECEIVABLE": { name: "Accounts Receivable", type: "asset" },
  "STOCK-IN-HAND": { name: "Stock-in-Hand", type: "asset" },

  // Liabilities (2000 - 2999) - Credit balances
  "ACCOUNTS PAYABLE": { name: "Accounts Payable", type: "liability" },
  "DUTIES & TAXES": { name: "GST/VAT Payable", type: "liability" },

  // Equity (3000 - 3999) - Credit balances
  "CAPITAL / OWNER EQUITY": { name: "Capital / Owner Equity", type: "equity" },
  "RETAINED EARNINGS": { name: "Retained Earnings", type: "equity" },

  // Revenue (4000 - 4999) - Credit balances
  "SALES A/C": { name: "Sales A/C", type: "revenue" },
  "SALES RETURNS": { name: "Sales Returns", type: "revenue" }, // Contra-revenue
  "SHIPPING REVENUE": { name: "Shipping Revenue", type: "revenue" },
  "INVENTORY ADJUSTMENT GAIN": { name: "Inventory Adjustment Gain", type: "revenue" },

  // Expenses (5000 - 5999) - Debit balances
  "COST OF GOODS SOLD (COGS)": { name: "Cost of Goods Sold (COGS)", type: "expense" },
  "PURCHASE RETURNS": { name: "Purchase Returns", type: "expense" }, // Contra-expense
  "TRANSPORT & DELIVERY EXPENSE": { name: "Transport & Delivery Expense", type: "expense" },
  "RENT & UTILITIES": { name: "Rent & Utilities", type: "expense" },
  "SALARIES & WAGES": { name: "Salaries & Wages", type: "expense" },
  "INVENTORY WRITE-OFF / LOSS": { name: "Inventory Write-Off / Loss", type: "expense" },
  "OFFICE EXPENSES": { name: "Office Expenses", type: "expense" }
};
...
class StateManager {
  ...
  migrateLegacyTaxEntries() {
    ...
    this.transactions.forEach(tx => {
      const hasLegacyTax = tx.entries.some(e => e.accountId === "DUTIES & TAXES");
      ...
      if (inv) {
        ...
        tx.entries = tx.entries.filter(e => e.accountId !== "DUTIES & TAXES");
        ...
        if (totalCess > 0) {
          tx.entries.push({ accountId: "DUTIES & TAXES", debit: 0, credit: totalCess });
        }
        stateChanged = true;
        return;
      }
      ...
      if (pur) {
        ...
        if (totalCess > 0) {
          tx.entries.push({ accountId: "DUTIES & TAXES", debit: totalCess, credit: 0 });
        }
        stateChanged = true;
        return;
      }
      ...
      tx.entries.forEach(entry => {
        if (entry.accountId === "DUTIES & TAXES") {
  ...
  rebuildAllTaxTransactions() {
    ...
    // Define prefix arrays at function scope so ALL steps (1, 3, 4, 5, 6) can reference them
    ...
    // 1. Completely clear all Duties & Taxes (groupName: "Duties & Taxes" / GST groups) and code "DUTIES & TAXES" entries from ALL invoice/purchase-related transactions
    this.transactions.forEach(tx => {
      ...
      if (isSales || isPurchase) {
        ...
        tx.entries = tx.entries.filter((entry) => {
          if (entry.accountId === "DUTIES & TAXES") return false;
          const led = this.ledgers.find(l => l.code === entry.accountId || (typeof entry.accountId === 'string' && l.code === entry.accountId.toUpperCase()));
          if (led) {
            const n = (led.name || "").toUpperCase();
            const g = (led.groupName || "").toUpperCase();
            // Keep base purchase/sales account ledgers (taxable value entries)
            if (g === "PURCHASE ACCOUNT" || g === "SALES ACCOUNT" || g === "PURCHASE ACCOUNTS" || g === "SALES ACCOUNTS") return true;
            // Keep cash/bank/sundry debtor/creditor ledgers (the payee/payer side)
            if (g === "CASH-IN-HAND" || g === "BANK ACCOUNTS" || g === "SUNDRY DEBTORS" || g === "SUNDRY CREDITORS" || g === "CURRENT ASSETS" || g === "CURRENT LIABILITIES") return true;
            // Remove any GST/tax entries — they will be freshly rebuilt
            if (g === "DUTIES & TAXES" || n.includes("CGST") || n.includes("SGST") || n.includes("IGST") || n.includes("CESS")) return false;
            const gstGroups = ["INPUT SGST", "INPUT CGST", "INPUT IGST", "OUTPUT SGST", "OUTPUT CGST", "OUTPUT IGST"];
            if (gstGroups.includes(g)) return false;
          }
          // For contact IDs (not ledger codes), always keep
          return true;
        });
    ...
    this.invoices.forEach(inv => {
      ...
      const tx = this.transactions.find(t => {
        ...
        // STRICTLY ENFORCE that this is a sales transaction
        const isSales = salesPrefixes.some(p => tid.startsWith(p) || tref.startsWith(p)) || tdesc.includes("SALES") || tdesc.includes("INVOICE");
      ...
      if (tx) {
        // Strip ALL existing GST/tax entries first to avoid duplicates and stale wrong entries
        tx.entries = tx.entries.filter(entry => {
          if (entry.accountId === "DUTIES & TAXES") return false;
          if (typeof entry.accountId === 'string') {
             const accUpper = entry.accountId.toUpperCase();
             const isAutoTax = (accUpper.startsWith("INPUT ") || accUpper.startsWith("OUTPUT ")) && (accUpper.includes("CGST") || accUpper.includes("SGST") || accUpper.includes("IGST") || accUpper.includes("CESS"));
             if (isAutoTax) return false;
          }
          const led = this.ledgers.find(l => l.code === entry.accountId || (typeof entry.accountId === 'string' && l.code === entry.accountId.toUpperCase()));
          if (!led) return true;
          const n = (led.name || "").toUpperCase();
          const g = (led.groupName || "").toUpperCase();
          if (g === "PURCHASE ACCOUNT" || g === "SALES ACCOUNT" || g === "PURCHASE ACCOUNTS" || g === "SALES ACCOUNTS") return true;
          if (g === "DUTIES & TAXES") return false;
          return true;
        });

        const invSeries = this.seriesMaster.find(s => s.id === inv.seriesId);
        const seriesType = invSeries ? invSeries.seriesType : "LOCAL";
        const isKeralaByState = (inv.state || companyState).toUpperCase() === companyState;
        const isIGSTBySeriesType = seriesType && ["INTERSTATE", "IGST", "OUTSTATE"].includes(seriesType.toUpperCase());
        const isLocal = isIGSTBySeriesType ? false : isKeralaByState;
        ...
        const hasSalesEntry = tx.entries.some(e => {
          const l = this.ledgers.find(x => x.code === e.accountId || x.name === e.accountId);
          return l && (l.groupName.toUpperCase() === "SALES ACCOUNT" || l.groupName.toUpperCase() === "SALES ACCOUNTS" || l.name.toUpperCase().includes("SALES"));
        });
        if (!hasSalesEntry) {
          const salAcc = isLocal ? "LOCAL SALES" : "IGST SALES";
          tx.entries.push({ accountId: salAcc, debit: 0, credit: (inv.subtotal - (inv.totalDiscount || 0)) });
        }
        stateChanged = true;
      } else {
    ...
    this.purchases.forEach(pur => {
      ...
      const tx = this.transactions.find(t => {
        ...
        // STRICTLY ENFORCE that this is a purchase transaction
        const isSales = salesPrefixes.some(p => tid.startsWith(p) || tref.startsWith(p)) || tdesc.includes("SALES") || tdesc.includes("INVOICE");
        const isPurchase = !isSales && (purchasePrefixes.some(p => tid.startsWith(p) || tref.startsWith(p)) || tdesc.includes("PURCHASE"));
      ...
      if (tx) {
        ...
        let seriesType = purSeries ? purSeries.seriesType : "";
        // If no series found, detect IGST by prefix (IPR- = Interstate Purchase) or seriesType on the series itself
        if (!seriesType) {
          const vno = String(pur.voucherNo || pur.id || "").toUpperCase();
          const igstPurchaseSeries = this.seriesMaster.find(s => s.txType === "Purchase" && s.seriesType && ["INTERSTATE","IGST","OUTSTATE"].includes(s.seriesType.toUpperCase()));
          if (igstPurchaseSeries && igstPurchaseSeries.prefix && vno.startsWith(igstPurchaseSeries.prefix.toUpperCase())) {
            seriesType = igstPurchaseSeries.seriesType;
          } else if (vno.startsWith("IPR")) {
            seriesType = "INTERSTATE";
          }
        }
        const isKeralaByState = (pur.state || companyState).toUpperCase() === companyState;
        const isIGSTBySeriesType = seriesType && ["INTERSTATE", "IGST", "OUTSTATE"].includes(seriesType.toUpperCase());
        // Also detect IGST by supplier's state being different from company state
        const supplier = this.contacts.find(c => c.id === pur.contactId);
        const supplierState = supplier ? (supplier.state || "").toUpperCase() : "";
        const isIGSTBySupplierState = supplierState && supplierState !== companyState;
        const isLocal = isIGSTBySeriesType || isIGSTBySupplierState ? false : isKeralaByState;
        ...
        const hasPurEntry = tx.entries.some(e => {
          const l = this.ledgers.find(x => x.code === e.accountId || x.name === e.accountId);
          return l && (l.groupName.toUpperCase() === "PURCHASE ACCOUNT" || l.groupName.toUpperCase() === "PURCHASE ACCOUNTS" || l.name.toUpperCase().includes("PURCHASE"));
        });
        if (!hasPurEntry) {
          const purAcc = isLocal ? "LOCAL PURCHASE" : "IGST PURCHASE";
          tx.entries.push({ accountId: purAcc, debit: (pur.subtotal - (pur.totalDiscount || 0)), credit: 0 });
        }
        stateChanged = true;
      } else {
  ...
  recreatePurchaseTransaction(pur) {
    ...
    if (!seriesType) {
      const vno = String(pur.voucherNo || pur.id || "").toUpperCase();
      if (vno.startsWith("IPR")) seriesType = "INTERSTATE";
      else seriesType = "LOCAL";
    }

    const isKeralaByState = (pur.state || this.getCompanyState()).toUpperCase() === this.getCompanyState();
    const isIGSTBySeriesType = seriesType && ["INTERSTATE", "IGST", "OUTSTATE"].includes(seriesType.toUpperCase());
    const supplierState = supplier ? (supplier.state || "").toUpperCase() : "";
    const isIGSTBySupplierState = supplierState && supplierState !== this.getCompanyState();
    const isLocal = isIGSTBySeriesType || isIGSTBySupplierState ? false : isKeralaByState;

    let purchaseDebitLedger = purSeries ? purSeries.ledgerCode : null;
    if (!purchaseDebitLedger || purchaseDebitLedger === "LOCAL PURCHASE" || purchaseDebitLedger === "L018") {
      if (seriesType === "NONTAXABLE") {
        const l = this.ledgers.find(x => x.name && x.name.toUpperCase() === "NON TAXABLE PURCHASE");
        purchaseDebitLedger = l ? l.name : "NON TAXABLE PURCHASE";
      } else if (!isLocal) {
        const l = this.ledgers.find(x => x.name && x.name.toUpperCase() === "IGST PURCHASE");
        purchaseDebitLedger = l ? l.name : "IGST PURCHASE";
      } else {
        const l = this.ledgers.find(x => x.name && x.name.toUpperCase() === "LOCAL PURCHASE");
        purchaseDebitLedger = l ? l.name : "LOCAL PURCHASE";
      }
    }
    ...
    const tx = {
      ...
      reference: pur.voucherNo || pur.refNo || pur.id,
      description: `Purchase from ${pur.contactName} - Ref: ${pur.refNo || "-"}`,
      siteName: pur.siteName || "",
      entries: [
        { accountId: purchaseDebitLedger, debit: (pur.subtotal - (pur.totalDiscount || 0)), credit: 0 },
    ...
    Object.keys(taxGroups).forEach(rateStr => {
      ...
      if (gstAmt > 0) {
        if (isLocal) {
          const cgstLedger = this.getOrCreateDutiesAndTaxesLedger(`Input CGST ${(rate / 2).toFixed(1).replace(".0", "")}%`);
          const sgstLedger = this.getOrCreateDutiesAndTaxesLedger(`Input SGST ${(rate / 2).toFixed(1).replace(".0", "")}%`);
          tx.entries.push({ accountId: cgstLedger, debit: gstAmt / 2, credit: 0 });
          tx.entries.push({ accountId: sgstLedger, debit: gstAmt / 2, credit: 0 });
        } else {
          const igstLedger = this.getOrCreateDutiesAndTaxesLedger(`Input IGST ${rate.toFixed(1).replace(".0", "")}%`);
          tx.entries.push({ accountId: igstLedger, debit: gstAmt, credit: 0 });
        }
    ...
    const totalCess = parseFloat(pur.totalCess || 0) + parseFloat(pur.additionalCess || 0);
    if (totalCess > 0) {
      tx.entries.push({ accountId: "DUTIES & TAXES", debit: totalCess, credit: 0 });
    }
    ...
    if (roundOff !== 0) {
      ...
      const roundName = roundL ? roundL.name : "ROUND OFF";
      if (roundOff > 0) {
        tx.entries.push({ accountId: roundName, debit: roundOff, credit: 0 });
      } else {
        tx.entries.push({ accountId: roundName, debit: 0, credit: Math.abs(roundOff) });
      }
    }

    if (pur.adjustmentsList && pur.adjustmentsList.length > 0) {
      pur.adjustmentsList.forEach(a => {
        ...
        if (amt > 0) {
          ...
          if (!ledgerCode) {
            ...
            ledgerCode = master ? master.ledgerCode : "TRANSPORT & DELIVERY EXPENSE";
          }
          if (a.type === "Add") {
  ...
  recreateInvoiceTransaction(inv) {
    ...
    if (inv.payMode === "Cash") {
      const cashL = this.ledgers.find(l => l.groupName === "CASH-IN-HAND" || l.name.toUpperCase() === "CASH");
      debitAccount = cashL ? cashL.name : "CASH";
    ...
    const salesCreditLedger = "LOCAL SALES";
    ...
    if (totalCess > 0) {
      salesEntries.push({ accountId: "DUTIES & TAXES", debit: 0, credit: totalCess });
    ...
    const roundL = this.ledgers.find(l => l.name.toUpperCase().includes("ROUND"));
    const roundCode = roundL ? roundL.name : "ROUND OFF";
  ...
  ensureStandardBaseLedgers() {
    ...
    standardBaseLedgers.forEach(stdL => {
      ...
      if (conflictIdx !== -1) {
        ...
        if (conflict.name.toUpperCase() !== stdL.name.toUpperCase()) {
        ...
        const nameIdx = this.ledgers.findIndex(l => l.name.toUpperCase() === stdL.name.toUpperCase());
        if (nameIdx === -1) {
          this.ledgers.push(JSON.parse(JSON.stringify(stdL)));
          changed = true;
        }
  ...
  healCorruptedBaseLedgers() {
    ...
    this.transactions.forEach(tx => {
      ...
      if (isPurchase) {
        ...
        const baseLedger = this.ledgers.find(l => l.code === baseLedgerCode || l.name === baseLedgerCode);
        if (!baseLedger || baseLedger.groupName.toUpperCase() === "DUTIES & TAXES") {
          ...
          let correctCode = "LOCAL PURCHASE";
          if (pur && pur.seriesId) {
  ...
  loadState(skipNotify = false) {
    try {
      ...
      this.syncFromFirebase(activeId);
      ...
      // Clean corrupted groups
      this.accountGroups = this.accountGroups.filter(g => g && g.name);

      // Ensure all standard initialAccountGroups exist
      if (typeof initialAccountGroups !== 'undefined') {
        initialAccountGroups.forEach(ig => {
          if (!this.accountGroups.some(g => g.name.toUpperCase() === ig.name.toUpperCase())) {
            this.accountGroups.push(JSON.parse(JSON.stringify(ig)));
            this.saveState();
          }
        });
      }
      ...
      const expectedParents = {
        ...
        const defaultLedgers = [
          { name: "SALES A/C", groupName: "SALES ACCOUNT", openingBalance: 0, balanceType: "Credit" },
          { name: "LOCAL PURCHASE", groupName: "PURCHASE ACCOUNT", openingBalance: 0, balanceType: "Debit" },
          { name: "IGST PURCHASE", groupName: "PURCHASE ACCOUNT", openingBalance: 0, balanceType: "Debit" },
          { name: "NON TAXABLE PURCHASE", groupName: "PURCHASE ACCOUNT", openingBalance: 0, balanceType: "Debit" },
          { name: "LOCAL SALES", groupName: "SALES ACCOUNT", openingBalance: 0, balanceType: "Credit" },
          { name: "IGST SALES", groupName: "SALES ACCOUNT", openingBalance: 0, balanceType: "Credit" },
          { name: "NON TAXABLE SALES", groupName: "SALES ACCOUNT", openingBalance: 0, balanceType: "Credit" },
          { name: "FREIGHT ON SALES", groupName: "ADJUSTMENTS", openingBalance: 0, balanceType: "Debit" },
          { name: "FREIGHT ON PURCHASE", groupName: "ADJUSTMENTS", openingBalance: 0, balanceType: "Debit" },
          { name: "LOADING CHARGE", groupName: "ADJUSTMENTS", openingBalance: 0, balanceType: "Debit" },
          { name: "UNLOADING CHARGE", groupName: "ADJUSTMENTS", openingBalance: 0, balanceType: "Debit" },
          { name: "ROUND OFF", groupName: "INDIRECT EXPENSES", openingBalance: 0, balanceType: "Debit" }
        ];
        ...
        const getLedgerNameByName = (name, fallbackName) => {
          const l = this.ledgers.find(x => x.name && x.name.toUpperCase() === name.toUpperCase());
          return l ? l.name : fallbackName;
        };

        // Set up / enforce default sales adjustments affecting their respective ledgers
        this.salesAdjustments = [
          { name: "FREIGHT", ledgerCode: getLedgerNameByName("FREIGHT ON SALES", "FREIGHT ON SALES") },
          { name: "LOADING CHARGE", ledgerCode: getLedgerNameByName("LOADING CHARGE", "LOADING CHARGE") },
          { name: "LOADING CHARGES", ledgerCode: getLedgerNameByName("LOADING CHARGE", "LOADING CHARGE") },
          { name: "UNLOADING CHARGE AT SITE", ledgerCode: getLedgerNameByName("UNLOADING CHARGE", "UNLOADING CHARGE") }
        ];

        // Set up / enforce default purchase adjustments affecting their respective ledgers
        this.purchaseAdjustments = [
          { name: "FREIGHT", ledgerCode: getLedgerNameByName("FREIGHT ON PURCHASE", "FREIGHT ON PURCHASE") },
          { name: "UNLOADING CHARGE", ledgerCode: getLedgerNameByName("UNLOADING CHARGE", "UNLOADING CHARGE") }
        ];
  ...
  resetToDefault() {
    ...
    this.salesAdjustments = [
      { name: "FREIGHT", ledgerCode: "FREIGHT ON SALES" },
      { name: "LOADING CHARGE", ledgerCode: "LOADING CHARGE" },
      { name: "LOADING CHARGES", ledgerCode: "LOADING CHARGE" },
      { name: "OTHER ITEMS", ledgerCode: "OTHER ITEMS" },
      { name: "UNLOADING CHARGE", ledgerCode: "UNLOADING CHARGE" }
    ];
    this.purchaseAdjustments = [
      { name: "FREIGHT", ledgerCode: "FREIGHT ON PURCHASE" },
      { name: "FREIGHT ON PURCHASE", ledgerCode: "FREIGHT ON PURCHASE" },
      { name: "UNLOADING CHARGE", ledgerCode: "UNLOADING CHARGE" }
    ];
  ...
  async syncFromFirebase(activeId) {
    try {
      ...
      if (docSnap.exists()) {
        ...
        // If local storage already has data, DO NOT overwrite it with Firebase data.
        // This prevents stale cloud data from wiping out local data if a cloud push was interrupted.
        const localHasData = this.materials.length > 0 || this.contacts.length > 0 || this.invoices.length > 0 || this.transactions.length > 0;
        if (localHasData) {
          // Push current local state to cloud to ensure cloud is up to date
          this.saveState(true);
          return;
        }

        // Update arrays from Firebase because local is empty (e.g., new device)
        this.materials = parsed.materials || [];
        this.contacts = parsed.contacts || [];
        this.invoices = parsed.invoices || [];
        ...
        // Overwrite local cache with cloud truth
        localStorage.setItem(`erp_company_data_${activeId}`, JSON.stringify(parsed));
        this.notifyListeners();
      }
    } catch (e) {
  ...
  ensureDefaultSeries() {
    ...
    const defaults = [
      ...
      },
      {
        ...
        startingNumber: 1,
        currentNumber: 1,
        ledgerCode: "NON TAXABLE PURCHASE",
        isActive: false
      }
    ];
  ...
  getStockAdjustments() {
   ...
   recreateStockAdjustmentTransaction(doc) {
     ...
     (doc.items || []).forEach(item => {
       ...
       if (amt > 0) {
         if (item.stockAffect === "Add (+)") {
           entries.push({ accountId: "STOCK-IN-HAND", debit: amt, credit: 0 });
           entries.push({ accountId: "INVENTORY ADJUSTMENT GAIN", debit: 0, credit: amt });
         } else {
           entries.push({ accountId: "INVENTORY WRITE-OFF / LOSS", debit: amt, credit: 0 });
           entries.push({ accountId: "STOCK-IN-HAND", debit: 0, credit: amt });
         }
       }
     });
     if (entries.length > 0) {
  ...
  addGstMasterEntry(entry) {
    ...
    if (!ledgerExists) {
      ledgers.push({
        code: newEntry.name.toUpperCase(),
        name: newEntry.name,
  ...
  deleteGstMasterEntry(id) {
    ...
    if (idx !== -1) {
      ...
      if (lIdx !== -1) {
        const ledgerName = ledgers[lIdx].name;
        const hasTx = this.transactions.some(tx => tx.entries.some(ent => ent.accountId === ledgerName));
        if (!hasTx) {
  ...
  cascadeRename(oldName, newName) {
    (this.transactions || []).forEach(tx => {
      (tx.entries || []).forEach(e => {
        ...
        if (baseId === oldName) {
          e.accountId = sitePart ? `${newName}::${sitePart}` : newName;
        }
      });
    });
    (this.seriesMaster || []).forEach(s => {
      if (s.ledgerCode === oldName) s.ledgerCode = newName;
    });
  }
  ...
  updateLedger(oldName, ledgerData) {
    const l = this.ledgers.find(x => x.name.toUpperCase() === oldName.toUpperCase());
    if (!l) return null;
    
    const conflict = this.ledgers.some(x => x.name.toUpperCase() !== oldName.toUpperCase() && x.name.toLowerCase() === ledgerData.name.toLowerCase());
    ...
    const newName = ledgerData.name.toUpperCase();
    if (oldName.toUpperCase() !== newName) {
      this.cascadeRename(oldName.toUpperCase(), newName);
      l.code = newName;
    }
  ...
  deleteLedger(name) {
    const existsInTransactions = this.transactions.some(tx => 
      tx.entries.some(e => e.accountId === name || e.accountId.startsWith(name + "::"))
    );
    const existsInPurchases = this.purchases.some(p => 
      p.adjustmentsList?.some(a => a.ledgerCode === name)
    );
    const existsInInvoices = this.invoices.some(i => 
      i.adjustmentsList?.some(a => a.ledgerCode === name)
    );
    ...
    const idx = this.ledgers.findIndex(l => l.name.toUpperCase() === name.toUpperCase());
    if (idx === -1) return false;
  ...
  getAccountBalances(endDate = "") {
    ...
    if (this.contacts) {
      this.contacts.forEach(c => {
        const controlAcc = (c.type === "supplier" || c.listInVendorList) ? "SUNDRY CREDITORS" : "SUNDRY DEBTORS";
        if (c.siteType === "multiple" && Array.isArray(c.sites) && c.sites.length > 0) {
    ...
    this.transactions.forEach(tx => {
      ...
      tx.entries.forEach(e => {
        ...
        if (contact) {
          const controlAcc = (contact.type === "supplier" || contact.listInVendorList) ? "SUNDRY CREDITORS" : "SUNDRY DEBTORS";
        ...
        if (matchedContact && (accId === "SUNDRY DEBTORS" || accId === "SUNDRY CREDITORS")) {
          const expectedControl = (matchedContact.type === "supplier" || matchedContact.listInVendorList) ? "SUNDRY CREDITORS" : "SUNDRY DEBTORS";
  ...
  ensureDefaultAdjustments() {
    ...
    const getLedgerNameByName = (name, fallbackName) => {
      if (!this.ledgers) return fallbackName;
      const l = this.ledgers.find(x => x.name && x.name.toUpperCase() === name.toUpperCase());
      return l ? l.name : fallbackName;
    };
  ...
  addContact(contactData) {
    ...
    const id = contactData.name.toUpperCase();
    const newContact = {
      id,
  ...
  updateContact(id, updatedData) {
    ...
    const newId = updatedData.name.toUpperCase();
    if (id !== newId) {
      this.cascadeRename(id, newId);
    }
  ...
  addMaterial(material) {
    ...
    if (openingStock > 0) {
      ...
      this.addTransactionInternal({
        ...
        entries: [
          { accountId: "STOCK-IN-HAND", debit: costAmount, credit: 0 },
          { accountId: "CAPITAL / OWNER EQUITY", debit: 0, credit: costAmount }
        ]
      });
    }
  ...
  updateMaterial(id, updatedData) {
    ...
    mat.mrp = parseFloat(updatedData.mrp) || mat.mrp || 0;
  ...
  adjustStock(materialId, adjustmentQty, reason, date) {
    ...
    if (qty > 0) {
      tx.entries.push({ accountId: "STOCK-IN-HAND", debit: costValue, credit: 0 });
      tx.entries.push({ accountId: "INVENTORY ADJUSTMENT GAIN", debit: 0, credit: costValue });
    } else {
      tx.entries.push({ accountId: "INVENTORY WRITE-OFF / LOSS", debit: costValue, credit: 0 });
      tx.entries.push({ accountId: "STOCK-IN-HAND", debit: 0, credit: costValue });
    }
    this.transactions.push(tx);
    this.saveState();
    return true;
  ...
  createStockConversion(convData) {
    ...
    this.transactions.push({
      ...
      entries: [
        { accountId: "STOCK-IN-HAND", debit: costValue, credit: 0 },
        { accountId: "STOCK-IN-HAND", debit: 0, credit: costValue }
      ]
    });
  ...
  recordPurchase(purchaseData) {
    ...
    if (purchaseData.payMode === "Cash") {
      const cashL = this.ledgers.find(l => l.groupName === "CASH-IN-HAND" || l.name.toUpperCase() === "CASH");
      creditAccount = cashL ? cashL.name : "CASH";
    ...
    const isKeralaByState = (purchaseData.state || supplier.state || this.getCompanyState()).toUpperCase() === this.getCompanyState();
    const isIGSTBySeriesType = seriesType && ["INTERSTATE", "IGST", "OUTSTATE"].includes(seriesType.toUpperCase());
    ...
    let purchaseDebitLedger = purchaseData.purchaseDebitLedger || "LOCAL PURCHASE";
    if (!purchaseData.purchaseDebitLedger || purchaseData.purchaseDebitLedger === "LOCAL PURCHASE") {
      if (seriesType === "NONTAXABLE") {
        const l = this.ledgers.find(x => x.name && x.name.toUpperCase() === "NON TAXABLE PURCHASE");
        purchaseDebitLedger = l ? l.name : "NON TAXABLE PURCHASE";
      } else if (!isKerala) {
    ...
    if (totalCess + additionalCess > 0) {
      tx.entries.push({ accountId: "DUTIES & TAXES", debit: (totalCess + additionalCess), credit: 0 });
    }
    ...
    if (adjustments !== 0 && (!purchaseData.adjustmentsList || purchaseData.adjustmentsList.length === 0)) {
      const shippingL = this.ledgers.find(l => l.name.toUpperCase().includes("SHIPPING") || l.name.toUpperCase().includes("TRANSPORT"));
      const shippingName = shippingL ? shippingL.name : "TRANSPORT & DELIVERY EXPENSE";
      if (adjustments > 0) {
        tx.entries.push({ accountId: shippingName, debit: adjustments, credit: 0 });
      } else {
        tx.entries.push({ accountId: shippingName, debit: 0, credit: Math.abs(adjustments) });
      }
    }
    ...
    if (purchaseData.adjustmentsList) {
      purchaseData.adjustmentsList.forEach(a => {
        const amt = parseFloat(a.amount) || 0;
        if (amt > 0) {
          let ledgerName = a.ledgerCode;
          if (!ledgerName) {
            const master = this.purchaseAdjustments.find(m => m.name.toUpperCase() === a.name.toUpperCase());
            ledgerName = master ? master.ledgerCode : "TRANSPORT & DELIVERY EXPENSE";
          }
          if (a.type === "Add") {
            tx.entries.push({ accountId: ledgerName, debit: amt, credit: 0 });
          } else {
            tx.entries.push({ accountId: ledgerName, debit: 0, credit: amt });
          }
        }
      });
    }
    ...
    this.purchases.push({
      ...
      roundOff: roundOff,
      total: itemsNetTotal,
      purchaseDebitLedger: purchaseDebitLedger
    });
  ...
  createPurchaseReturn(returnData) {
    ...
    let purchaseLedgerCode = "LOCAL PURCHASE";
    if (!hasTax) {
      const l = this.ledgers.find(x => x.name && x.name.toUpperCase() === "NON TAXABLE PURCHASE");
      purchaseLedgerCode = l ? l.name : "NON TAXABLE PURCHASE";
    } else if (isKerala) {
      const l = this.ledgers.find(x => x.name && x.name.toUpperCase() === "LOCAL PURCHASE");
      purchaseLedgerCode = l ? l.name : "LOCAL PURCHASE";
    } else {
      const l = this.ledgers.find(x => x.name && x.name.toUpperCase() === "IGST PURCHASE");
      purchaseLedgerCode = l ? l.name : "IGST PURCHASE";
    }
    ...
    if (roundOff !== 0) {
      ...
      const roundName = roundL ? roundL.name : "ROUND OFF";
      if (roundOff > 0) {
        entries.push({ accountId: roundName, debit: 0, credit: roundOff });
      } else {
        entries.push({ accountId: roundName, debit: Math.abs(roundOff), credit: 0 });
      }
    }
  ...
  createSalesReturn(returnData) {
    ...
    let hasTax = Object.values(taxGroups).some(gstAmt => gstAmt > 0);
    let salesLedgerCode = "LOCAL SALES";
    if (!hasTax) {
      const l = this.ledgers.find(x => x.name && x.name.toUpperCase() === "NON TAXABLE SALES");
      salesLedgerCode = l ? l.name : "NON TAXABLE SALES";
    } else if (isKerala) {
      const l = this.ledgers.find(x => x.name && x.name.toUpperCase() === "LOCAL SALES");
      salesLedgerCode = l ? l.name : "LOCAL SALES";
    } else {
      const l = this.ledgers.find(x => x.name && x.name.toUpperCase() === "IGST SALES");
      salesLedgerCode = l ? l.name : "IGST SALES";
    }
    ...
    if (finalRoundOff > 0) {
      entries.push({ accountId: roundName, debit: finalRoundOff, credit: 0 });
      runningDebit += finalRoundOff;
    } else if (finalRoundOff < 0) {
      entries.push({ accountId: roundName, debit: 0, credit: Math.abs(finalRoundOff) });
    ...
    if (diff !== 0) {
        if (diff > 0) {
            entries.push({ accountId: roundName, debit: 0, credit: diff });
        } else {
            entries.push({ accountId: roundName, debit: Math.abs(diff), credit: 0 });
        }
    }
    ...
    if (cogsReversal > 0) {
      ...
      this.transactions.push({
        ...
        siteName: returnData.siteName || "",
        entries: [
          { accountId: "STOCK-IN-HAND", debit: cogsReversal, credit: 0 },
          { accountId: "COST OF GOODS SOLD (COGS)", debit: 0, credit: cogsReversal }
        ]
      });
    }
  ...
  createInvoice(invoiceData) {
    ...
    const isKeralaByState = (invoiceData.state || customer.state || this.getCompanyState()).toUpperCase() === this.getCompanyState();
    const isIGSTBySeriesType = seriesType && ["INTERSTATE", "IGST", "OUTSTATE"].includes(seriesType.toUpperCase());
    const isKerala = isIGSTBySeriesType ? false : isKeralaByState;
    const taxGroups = {};
    items.forEach(item => {
    ...
    const salesCreditLedger = (series ? series.ledgerCode : null) || "LOCAL SALES";
    const salesEntries = [
    ...
    if (totalCess + additionalCess > 0) {
      salesEntries.push({ accountId: "DUTIES & TAXES", debit: 0, credit: (totalCess + additionalCess) });
    }
    ...
    if (roundOff !== 0) {
      ...
      const roundName = roundL ? roundL.name : "ROUND OFF";
      if (roundOff > 0) {
        salesEntries.push({ accountId: roundName, debit: 0, credit: roundOff });
      } else {
        salesEntries.push({ accountId: roundName, debit: Math.abs(roundOff), credit: 0 });
      }
    }

    if (adjustments !== 0 && (!invoiceData.adjustmentsList || invoiceData.adjustmentsList.length === 0)) {
      const shippingL = this.ledgers.find(l => l.name.toUpperCase().includes("SHIPPING") || l.name.toUpperCase().includes("REVENUE"));
      const shippingName = shippingL ? shippingL.name : "SHIPPING REVENUE";
      if (adjustments > 0) {
        salesEntries.push({ accountId: shippingName, debit: 0, credit: adjustments });
      } else {
        salesEntries.push({ accountId: shippingName, debit: Math.abs(adjustments), credit: 0 });
      }
    }
    ...
    if (invoiceData.adjustmentsList) {
      invoiceData.adjustmentsList.forEach(a => {
        ...
        if (amt > 0) {
          let ledgerName = a.ledgerCode;
          if (!ledgerName || ledgerName === "OTHER ITEMS") {
            if (a.name.toUpperCase() === "LOADING CHARGES" || a.name.toUpperCase() === "LOADING CHARGE") {
              ledgerName = "LOADING CHARGE";
            } else if (a.name.toUpperCase() === "UNLOADING CHARGE" || a.name.toUpperCase() === "UNLOADING CHARGE AT SITE") {
              ledgerName = "UNLOADING CHARGE";
            } else if (a.name.toUpperCase() === "FREIGHT" || a.name.toUpperCase() === "FREIGHT ON SALES") {
              ledgerName = "FREIGHT ON SALES";
            } else {
              const master = this.salesAdjustments.find(m => m.name.toUpperCase() === a.name.toUpperCase());
              ledgerName = master ? master.ledgerCode : "OTHER ITEMS";
            }
            a.ledgerCode = ledgerName;
          }
          if (a.type === "Add") {
            salesEntries.push({ accountId: ledgerName, debit: 0, credit: amt });
          } else {
            salesEntries.push({ accountId: ledgerName, debit: amt, credit: 0 });
          }
        }
      });
    }
    ...
    if (totalCogs > 0) {
      ...
      this.transactions.push({
        ...
        entries: [
          { accountId: "COST OF GOODS SOLD (COGS)", debit: totalCogs, credit: 0 },
          { accountId: "STOCK-IN-HAND", debit: 0, credit: totalCogs }
        ]
      });
    }
  ...
  createReceiptVoucher(data) {
    ...
    const entries = [];
    if (cashAmt > 0) {
      entries.push({ accountId: data.cashAccountId || "CASH", debit: cashAmt, credit: 0 });
    }
    if (bankAmt > 0) {
      entries.push({ accountId: data.bankAccountId || "BANK CURRENT ACCOUNT", debit: bankAmt, credit: 0 });
    }
    if (entries.length === 0) {
  ...
  createPaymentVoucher(data) {
    ...
    if (cashAmt > 0) {
      entries.push({ accountId: data.cashAccountId || "CASH", debit: 0, credit: cashAmt });
    }
    if (bankAmt > 0) {
      entries.push({ accountId: data.bankAccountId || "BANK CURRENT ACCOUNT", debit: 0, credit: bankAmt });
    }
    if (entries.length === 1) {
  ...
  repairMissingTransactions() {
    ...
    if (this.purchases) {
      this.purchases.forEach(pur => {
        ...
        let repairDebitLedger = pur.purchaseDebitLedger || "LOCAL PURCHASE";
        
        if (!pur.purchaseDebitLedger || pur.purchaseDebitLedger === "LOCAL PURCHASE") {
          ...
          if (repairSeriesType === "NONTAXABLE") {
            const l = this.ledgers.find(x => x.name && x.name.toUpperCase() === "NON TAXABLE PURCHASE");
            repairDebitLedger = l ? l.name : "NON TAXABLE PURCHASE";
          } else if (repairSeriesType === "INTERSTATE" || repairSeriesType === "IGST" || repairSeriesType === "OUTSTATE" || !repairIsKerala) {
            const l = this.ledgers.find(x => x.name && x.name.toUpperCase() === "IGST PURCHASE");
            repairDebitLedger = l ? l.name : "IGST PURCHASE";
          } else {
            const l = this.ledgers.find(x => x.name && x.name.toUpperCase() === "LOCAL PURCHASE");
            repairDebitLedger = l ? l.name : "LOCAL PURCHASE";
          }
        }
        ...
        if (adjustments !== 0 && (!pur.adjustmentsList || pur.adjustmentsList.length === 0)) {
          ...
          const shippingName = shippingL ? shippingL.name : "TRANSPORT & DELIVERY EXPENSE";
          if (adjustments > 0) {
            entries.push({ accountId: shippingName, debit: adjustments, credit: 0 });
          } else {
            entries.push({ accountId: shippingName, debit: 0, credit: Math.abs(adjustments) });
          }
        }
        ...
        if (roundOff !== 0) {
          ...
          const roundName = roundL ? roundL.name : "ROUND OFF";
          if (roundOff > 0) {
            entries.push({ accountId: roundName, debit: roundOff, credit: 0 });
          } else {
            entries.push({ accountId: roundName, debit: 0, credit: Math.abs(roundOff) });
          }
        }
        ...
        if (pur.adjustmentsList) {
          pur.adjustmentsList.forEach(a => {
            ...
            if (amt > 0) {
              ...
              if (a.type === "Add") {
                entries.push({ accountId: ledgerName, debit: amt, credit: 0 });
              } else {
                entries.push({ accountId: ledgerName, debit: 0, credit: amt });
              }
            }
          });
        }
        ...
        if (tx) {
          ...
          this.transactions.push({
            ...
            description: `Purchase of building materials from ${pur.contactName} - Invoice: ${pur.invoiceNo || "-"}`,
            siteName: pur.siteName || "",
            entries
          });
          stateChanged = true;
    ...
    if (this.invoices) {
      this.invoices.forEach(inv => {
        ...
        // Resolve sales ledger dynamically based on stored salesCreditLedger (or default SALES A/C)
        const activeSalesSeries = inv.seriesId ? this.seriesMaster?.find(s => s.id === inv.seriesId) : null;
        const salesCreditLedger = (activeSalesSeries ? activeSalesSeries.ledgerCode : null) || "SALES A/C";
        ...
        const totalCess = (inv.totalCess || 0) + (inv.additionalCess || 0);
        if (totalCess > 0) {
          entries.push({ accountId: "DUTIES & TAXES", debit: 0, credit: totalCess });
        }
        ...
        if (adjustments !== 0 && (!inv.adjustmentsList || inv.adjustmentsList.length === 0)) {
          ...
          const shippingName = shippingL ? shippingL.name : "SHIPPING REVENUE";
          if (adjustments > 0) {
            entries.push({ accountId: shippingName, debit: 0, credit: adjustments });
          } else {
            entries.push({ accountId: shippingName, debit: Math.abs(adjustments), credit: 0 });
          }
        }
    ...
    if (this.salesReturns) {
      this.salesReturns.forEach(sr => {
        ...
        if (sr.payMode === "Cash") {
          const cashL = this.ledgers.find(l => l.groupName === "CASH-IN-HAND" || l.name.toUpperCase() === "CASH");
          customerAccount = cashL ? cashL.name : "CASH";
        } else if (sr.payMode === "Bank") {
          const bankL = this.ledgers.find(l => l.groupName === "BANK ACCOUNTS" || l.name.toUpperCase().includes("BANK"));
          customerAccount = bankL ? bankL.name : "BANK CURRENT ACCOUNT";
        }
    ...
    if (this.purchaseReturns) {
      this.purchaseReturns.forEach(pr => {
        ...
        if (pr.payMode === "Cash") {
          const cashL = this.ledgers.find(l => l.groupName === "CASH-IN-HAND" || l.name.toUpperCase() === "CASH");
          supplierAccount = cashL ? cashL.code : "CASH-IN-HAND";
        } else if (pr.payMode === "Bank") {
          const bankL = this.ledgers.find(l => l.groupName === "BANK ACCOUNTS" || l.name.toUpperCase().includes("BANK"));
          supplierAccount = bankL ? bankL.code : "BANK CURRENT ACCOUNT";
        }
  ...
  getProfitLoss(startDate = "", endDate = "") {
    ...
    const getRootCategory = (gName) => {
      if (!gName) return null;
      let current = String(gName).toUpperCase();
      const allGroups = this.accountGroups || [];
      const roots = ["ASSETS", "LIABILITIES", "EQUITY", "INCOME", "EXPENSE"];
      for (let i = 0; i < 15; i++) {
        if (roots.includes(current)) return current;
        // ADJUSTMENTS is under CURRENT LIABILITIES, let it traverse normally
    ...
    filteredTxs.forEach(tx => {
      tx.entries.forEach(e => {
        const id = e.accountId || e.accountName;
    ...
    const getAccountDetails = (accId) => {
      const led = ledgers.find(l => String(l.code).toUpperCase() === String(accId).toUpperCase() || String(l.name).toUpperCase() === String(accId).toUpperCase());
      if (led) return { name: led.name, groupName: led.groupName || "", balanceType: led.balanceType || "Debit" };
      return { name: accId, groupName: "", balanceType: "Debit" };
    };
    ...
    Object.keys(periodChanges).forEach(accId => {
      ...
      const isContact = this.contacts?.some(c => String(c.id) === String(accId) || String(accId).startsWith(String(c.id) + "::")) || String(accId).includes("::");
      if (isContact) return;

      const details = getAccountDetails(accId);
      const root = getRootCategory(details.groupName);
      
      let isIncome = false;
      let isExpense = false;
      
      if (root === "INCOME") isIncome = true;
      else if (root === "EXPENSE") isExpense = true;
      else if (root === "ADJUSTMENTS") isExpense = true;
      else if (!root && details.balanceType === "Debit") isExpense = true;
      else if (!root && details.balanceType === "Credit") isIncome = true;
      else return;

      const groupUpper = String(details.groupName || "").toUpperCase();
      
      if (isIncome) {
        if (groupUpper === "SALES ACCOUNT") {
          netSales += (-changeDb);
        } else {
          otherIncomes.push({ name: details.name, amount: -changeDb });
        }
      } else if (isExpense) {
        if (groupUpper === "PURCHASE ACCOUNT") {
          netPurchase += changeDb;
        } else {
          otherExpenses.push({ name: details.name, amount: changeDb });
        }
      }
    });
    ...
    const indirectIncomesTotal = otherIncomes.reduce((sum, i) => sum + i.amount, 0);
    const indirectExpensesTotal = otherExpenses.reduce((sum, e) => sum + e.amount, 0);
    
    const netProfit = grossProfit + indirectIncomesTotal - indirectExpensesTotal;

    const tradingDebitTotal = openingStock + netPurchase + (grossProfit > 0 ? grossProfit : 0);
    const tradingCreditTotal = netSales + closingStock + (grossProfit < 0 ? Math.abs(grossProfit) : 0);
    
    const plDebitTotal = (grossProfit < 0 ? Math.abs(grossProfit) : 0) + indirectExpensesTotal + (netProfit > 0 ? netProfit : 0);
    const plCreditTotal = (grossProfit > 0 ? grossProfit : 0) + indirectIncomesTotal + (netProfit < 0 ? Math.abs(netProfit) : 0);
  ...
  getBalanceSheet(startDate = "", endDate = "") {
    ...
    const getRootCategory = (groupName) => {
      if (!groupName) return null;
      let current = String(groupName).toUpperCase();
      const allGroups = this.accountGroups || [];
      const roots = ["ASSETS", "LIABILITIES", "EQUITY", "INCOME", "EXPENSE"];
      for (let i = 0; i < 15; i++) {
        if (roots.includes(current)) return current;
    ...
    let fixedAssetsVal = 0, fixedAssetsDetails = [];
    let currentAssetsVal = 0, currentAssetsDetails = [];
    let otherCurrentAssetsVal = 0, otherCurrentAssetsDetails = [];
    
    let capitalVal = 0, capitalDetails = [];
    let currentLiabilitiesVal = 0, currentLiabilitiesDetails = [];

    let diffInOpening = 0;

    const ledgers = this.getLedgers();
    Object.keys(balances).forEach(accId => {
      const balance = balances[accId].balance;
      if (Math.abs(balance) < 0.001) return;
      if (accId === "Difference in Opening Balance") {
        diffInOpening = balance;
        return;
      }
      ...
      if (led) {
        ...
        if (staticAcc) {
          name = staticAcc.name;
          if (staticAcc.type === "asset") groupName = "CURRENT ASSETS";
          if (staticAcc.type === "liability") groupName = "CURRENT LIABILITIES";
          if (staticAcc.type === "equity") groupName = "CAPITAL ACCOUNT";
        }
      }

      const root = getRootCategory(groupName);
      if (root === "INCOME" || root === "EXPENSE" || root === "ADJUSTMENTS") return;
      
      const groupUpper = String(groupName || "").toUpperCase();

      if (root === "ASSETS") {
        if (groupUpper === "FIXED ASSETS") {
          fixedAssetsVal += balance;
          fixedAssetsDetails.push({ name: name, balance: balance });
        } else if (groupUpper === "CURRENT ASSETS" || groupUpper === "BANK ACCOUNTS" || groupUpper === "CASH-IN-HAND" || groupUpper === "SUNDRY DEBTORS") {
          currentAssetsVal += balance;
          currentAssetsDetails.push({ name: name, balance: balance });
        } else {
          otherCurrentAssetsVal += balance;
          otherCurrentAssetsDetails.push({ name: name, balance: balance });
        }
      } else if (root === "LIABILITIES" || root === "EQUITY") {
        if (groupUpper === "CAPITAL ACCOUNT" || root === "EQUITY") {
          capitalVal += (-balance);
          capitalDetails.push({ name: name, balance: -balance });
        } else {
          currentLiabilitiesVal += (-balance);
          currentLiabilitiesDetails.push({ name: name, balance: -balance });
        }
      } else {
        if (balance > 0) {
          otherCurrentAssetsVal += balance;
    ...
    const pl = this.getProfitLoss(startDate, endDate);
    const netProfit = pl.netProfit || 0;

    currentAssetsVal += pl.closingStock;
    currentAssetsDetails.push({ name: "Closing Stock", balance: pl.closingStock });

    const totalAssets = fixedAssetsVal + currentAssetsVal + otherCurrentAssetsVal + (diffInOpening > 0 ? diffInOpening : 0) + (netProfit < 0 ? Math.abs(netProfit) : 0);
    const totalLiabilities = capitalVal + currentLiabilitiesVal + (diffInOpening < 0 ? Math.abs(diffInOpening) : 0) + (netProfit > 0 ? netProfit : 0);

    return {
      assets: {
        fixedAssetsVal, fixedAssetsDetails,
        currentAssetsVal, currentAssetsDetails,
        otherCurrentAssetsVal, otherCurrentAssetsDetails,
        total: totalAssets
      },
      liabilities: {
        capitalVal, capitalDetails,
        currentLiabilitiesVal, currentLiabilitiesDetails,
      ...
      tx.entries.forEach(entry => {
        if (entry.accountId === "DUTIES & TAXES") {
          const hasDynamicTax = tx.entries.some(e => {
            if (e.accountId === "DUTIES & TAXES") return false;
  ...
  getGroupAccounts(groupName) {
    ...
    if (targetGroups.includes("BANK ACCOUNTS")) {
      list.push("BANK CURRENT ACCOUNT");
    }

    if (targetGroups.includes("DUTIES & TAXES")) {
      list.push("DUTIES & TAXES");
    }

    if (targetGroups.includes("CAPITAL ACCOUNT")) {
      list.push("CAPITAL / OWNER EQUITY");
    }
  ...
  getAccountGroupName(accId) {
    const staticAcc = ACCOUNTS[accId];
    if (staticAcc) {
      if (accId === "CASH-IN-HAND") return "CASH-IN-HAND";
      if (accId === "BANK CURRENT ACCOUNT") return "BANK ACCOUNTS";
      if (accId === "SUNDRY DEBTORS") return "SUNDRY DEBTORS";
      if (accId === "1200") return "STOCK IN HAND";
      if (accId === "SUNDRY CREDITORS") return "SUNDRY CREDITORS";
      if (accId === "DUTIES & TAXES") return "DUTIES & TAXES";
      if (accId === "CAPITAL / OWNER EQUITY") return "CAPITAL ACCOUNT";
      ...
      if (accId === "1200") {
        ...
        this.transactions.forEach(tx => {
          ...
          tx.entries.forEach(e => {
            ...
            if ((targetAcc === "SUNDRY DEBTORS" || targetAcc === "SUNDRY CREDITORS") && matchedContact) {
              const suffix = matchedSite ? `::${matchedSite}` : "";
              targetAcc = matchedContact.id + suffix;
            }

            const accName = this.getAccountName(accId);
            if (targetAcc === accId || targetAcc.toUpperCase() === accId.toUpperCase() || (accName && targetAcc.toUpperCase() === accName.toUpperCase())) {
