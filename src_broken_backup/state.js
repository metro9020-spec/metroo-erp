...
class StateManager {
  ...
  rebuildAllTaxTransactions() {
    ...
    this.transactions.forEach(tx => {
      ...
      if (isSales || isPurchase) {
        ...
        tx.entries = tx.entries.filter((entry, idx) => {
          if (idx < 2) return true; // ALWAYS preserve the primary base ledger (index 0) and the vendor/customer ledger (index 1) for purchases/invoices!
  ...
  async syncFromServer() {
    ...
    if (!activeId) return;
    try {
      const res = await fetch(`http://${window.location.hostname}:3001/api/data/${activeId}/default`);
      if (res.ok) {
  ...
  saveState(skipNotify = false) {
    try {
      ...
      fetch(`http://${window.location.hostname}:3001/api/data/${activeId}/default`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  recordPurchase(purchaseData) {
    ...
    if (seriesType === "INTERSTATE" || seriesType === "IGST" || seriesType === "OUTSTATE" || !isKerala) {
