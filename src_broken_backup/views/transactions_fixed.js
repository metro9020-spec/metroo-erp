  ...
  container.innerHTML = `
    ...
    <div style="display: flex; gap: 0.5rem; border-bottom: 1px solid var(--border-color); padding-bottom: 0.5rem; flex-wrap: wrap;">
      ...
      <button class="btn ${activeSubTab === 'sales-return' ? 'btn-primary' : 'btn-secondary'} subtab-btn" data-subtab="sales-return">
        <i class="fa-solid fa-reply"></i> Sales Returns (Credit Note)
      </button>
      ...
      <button class="btn ${activeSubTab === 'purchase-return' ? 'btn-primary' : 'btn-secondary'} subtab-btn" data-subtab="purchase-return">
        <i class="fa-solid fa-reply-all"></i> Purchase Returns (Debit Note)
      </button>
      ...
      <button class="btn ${activeSubTab === 'stock-conversion' ? 'btn-primary' : 'btn-secondary'} subtab-btn" data-subtab="stock-conversion">
        <i class="fa-solid fa-boxes-packing"></i> Stock Loosening (Conversion)
      </button>
    </div>

    <!-- Active View Area -->
    <div id="transactions-subtab-content" style="margin-top: 1rem;">
      <!-- Dynamic subtab content goes here -->
    </div>
  `;

  const subtabContentEl = document.getElementById("transactions-subtab-content");
...
function renderSalesReturnSubTab(container) {
  const returns = state.getSalesReturns();
  
  container.innerHTML = `
    <div class="action-header" style="margin-bottom: 1rem;">
      <h3 style="font-size: 1.15rem;">Sales Returns (Credit Notes)</h3>
    ...
    <div class="panel" style="padding-top: 0.5rem;">
      <div class="table-responsive">
        <table class="data-table">
          ...
          <tbody>
            ${returns.length === 0 ? `
              <tr><td colspan="7" style="text-align: center; color: var(--text-muted); padding: 2rem;">No credit notes recorded.</td></tr>
            ` : returns.map(ret => `
              <tr class="sales-return-row" data-id="${ret.id}" style="cursor: pointer;" title="Double click to edit this credit note">
                ...
                <td style="text-align: center;">
                  <button class="btn btn-secondary btn-icon view-return-btn" data-id="${ret.id}" title="View Credit Note Details"><i class="fa-solid fa-eye"></i> View</button>
                </td>
              </tr>
            `).join("")}
          </tbody>
...
export function showInvoiceBuilderModal(container, customers = null, materials = null, onSuccess = null, editInvoice = null, selectedSeries = null) {
  ...
  let colsCount = 6; // Invoice No, Customer, State, Currency, Inv Date, Due Date
    ...
    <div class="modal-overlay active" id="modal-overlay" style="display:flex; justify-content:center; align-items:center; background: rgba(15,23,42,0.3); backdrop-filter: blur(1px); z-index:2000;">
      <div class="modal-container modal-lg" style="position: relative; max-width:1600px; width: 98vw; background-color:#cbd5e1; color:#0f172a; padding:15px; font-family: var(--font-body); border: 2px solid #5a7b9c; border-radius: 6px; box-shadow: 0 10px 40px rgba(0,0,0,0.3); font-size:0.85rem;">
        ...
        <form id="create-invoice-form" style="display:flex; flex-direction:column; gap:10px; margin-top:8px;">
          ...
          <div style="background-color:#f1f5f9; padding:10px; border:1px solid #94a3b8; border-radius:4px; display:grid; grid-template-columns: repeat(${colsCount}, 1fr); gap:10px; align-items:center;">
            ...
            <div>
              ...
              <select id="inv-currency" class="form-control" style="background-color:#e2e8f0; color:black; padding:2px 6px; font-size:0.8rem;" disabled>
  ...
  const curIdx = editInvoice ? allInvoices.findIndex(inv => inv.id === editInvoice.id) : -1;

  document.getElementById("btn-inv-prev").addEventListener("click", () => {
    if (curIdx > 0) {
      close();
      showInvoiceBuilderModal(container, customers, materials, onSuccess, allInvoices[curIdx - 1]);
    } else if (curIdx === -1 && allInvoices.length > 0) {
      close();
      showInvoiceBuilderModal(container, customers, materials, onSuccess, allInvoices[allInvoices.length - 1]);
    }
  });

  document.getElementById("btn-inv-next").addEventListener("click", () => {
    if (editInvoice && curIdx !== -1 && curIdx < allInvoices.length - 1) {
      close();
      showInvoiceBuilderModal(container, customers, materials, onSuccess, allInvoices[curIdx + 1]);
    }
  });
  ...
  document.getElementById("btn-inv-search").addEventListener("click", () => {
    ...
    searchOverlay.innerHTML = `
      <div style="background: #cbd5e1; border: 2px solid #1e3b8b; border-radius: 4px; width: 900px; padding: 8px; font-family: sans-serif; font-size: 0.8rem; color: black; display: flex; flex-direction: column; gap: 8px; box-shadow: 0 4px 20px rgba(0,0,0,0.4);">
        ...
        <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 6px; background: #e2e8f0; padding: 6px; border: 1px solid #94a3b8; border-radius: 2px;">
          ...
          <div>
            <label style="display: block; font-weight: bold; margin-bottom: 2px;">Reference No.</label>
            <input type="text" id="filter-refno" class="form-control" style="padding: 2px; font-size: 0.8rem; width: 100%;">
          </div>
        </div>
          ...
          <table style="width: 100%; border-collapse: collapse; text-align: left; font-size: 0.75rem; color: black;">
            <thead>
              <tr style="background: #1e3b8b; color: white; font-weight: bold; position: sticky; top: 0; z-index: 10;">
                ...
                <th style="padding: 4px 6px; border: 1px solid #cbd5e1;">Ref.No</th>
    ...
    const runSearch = () => {
      ...
      const refnoFilter = searchOverlay.querySelector("#filter-refno").value.toLowerCase();
      ...
      filtered = filtered.filter(x => {
        ...
        const matchesRefNo = extended ? (x.refNo || '').toLowerCase().includes(refnoFilter) : (x.refNo || '').toLowerCase().startsWith(refnoFilter);
        ...
        return matchesBillNo && matchesCustomer && matchesPaymode && matchesRefNo && matchesNetAmt;
      });
      ...
      ` : filtered.map(item => `
        <tr class="log-row-item" data-id="${item.id}" style="border-bottom: 1px solid #e2e8f0; cursor: pointer; user-select: none;" onmouseover="this.style.background='#f1f5f9'" onmouseout="this.style.background='white'">
          ...
          <td style="padding: 6px;">${item.refNo || ''}</td>
    ...
    const textInputs = ["#filter-billno", "#filter-customer", "#filter-paymode", "#filter-netamt", "#filter-refno"];
  ...
  invCustomerSelect.addEventListener("change", () => {
    ...
    if (isCashSales) {
      ...
      if (paymodeSelect) {
        ...
        if (!editInvoice) paymodeSelect.value = "Credit";
      }
    }
      ...
      if (mat) {
        ...
        if (typeof ribbonBatchSelect !== 'undefined' && ribbonBatchSelect) {
          ...
          if (batches && batches.length > 0) {
            ribbonBatchSelect.innerHTML = '<option value="">-- Batch --</option>' +
              batches.map(b => `<option value="${b.batchNo}">${b.batchNo} (Qty: ${b.quantity})</option>`).join("");
  ...
  function renderGridAndRecalc() {
    ...
    tbody.querySelectorAll(".sales-grid-row").forEach(row => {
      row.addEventListener("dblclick", () => {
        ...
        populateRibbonSaleBatches(item.materialId, item.batchNo);
...
export function showSalesReturnModal(container, editReturn = null, onSuccess = null) {
  const root = document.getElementById("modal-container-root");
...
export function showRecordPurchaseModal(container, editPurchase = null, onSuccess = null, selectedSeries = null) {
  ...
  const populateRibbonBatches = (materialId, selectedBatchNo = null) => {
    ...
    ribbonBatchSelect.innerHTML = '<option value="">-- Batch --</option>' +
      batches.map(b => `<option value="${b.batchNo}" ${selectedBatchNo === b.batchNo ? 'selected' : ''}>₹${b.landingCost} (Sell: ₹${b.sellingPrice})</option>`).join("");
    ribbonBatchSelect.disabled = false;
...
export function showPurchaseReturnModal(container, editReturn = null, onSuccess = null) {
  const root = document.getElementById("modal-container-root");
  const suppliers = state.getContacts().filter(c => c.type === "supplier" || c.listInVendorList === true);
  const materials = state.getMaterials();
  const salesExecutivesList = ["Mr. Suresh Kumar", "Mr. Rajesh P.", "Mr. Anil Nair", "Mrs. Bindu V."];
  ...
  root.innerHTML = `
    <div class="modal-overlay active" id="modal-overlay" style="display:flex; justify-content:center; align-items:center; background: rgba(15, 23, 42, 0.6); backdrop-filter: blur(8px); z-index:2000; padding: 20px;">
