...
export function showSeriesMasterModal(container) {
  ...
  function render() {
    ...
    root.innerHTML = `
      <div class="modal-overlay active" id="series-master-modal" style="display:flex; align-items:center; justify-content:center; background:rgba(0,0,0,0.55); position:fixed; inset:0; z-index:9999;">
        <div class="modal-container" style="pointer-events: auto; background:#b0c4de; border:2px solid #3b629b; border-radius:4px; width:720px; max-width:98vw; max-height:95vh; display:flex; flex-direction:column; font-family:Tahoma,sans-serif; font-size:12px; box-shadow:0 8px 32px rgba(0,0,0,0.4); color:#000;">
          ...
          <div style="display:flex; justify-content:flex-end; gap:4px; padding:6px 10px; background:#b4c6e0; border-top:1px solid #8ca6c8;">
  ...
  function bindEvents() {
    ...
    document.getElementById("sm-series-type")?.addEventListener("change", e => {
      ...
      const ledgerSelect = document.getElementById("sm-ledger");
      const isPurchase = selectedTxType.toLowerCase().includes("purchase");
      ...
      if (ledgerSelect) {
        const targetName = nameInput ? nameInput.value.split("(")[0].trim() : "";
        const ledgers = state.getLedgers() || [];
        const match = ledgers.find(l => l.name === targetName || l.name.includes(typeVal));
        if (match) ledgerSelect.value = match.code;
      }
    });
