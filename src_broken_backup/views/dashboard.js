...
export function renderDashboard(container) {
    ...
    <div class="metrics-grid">
      <div class="metric-card sales">
        ...
        <div class="metric-details">
          ...
          <span class="metric-value">$${pl.revenue.total.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
        </div>
      </div>
      <div class="metric-card expenses">
        ...
        <div class="metric-details">
          ...
          <span class="metric-value">$${pl.expenses.total.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
        </div>
      </div>
      <div class="metric-card cash">
        ...
        <div class="metric-details">
          ...
          <span class="metric-value ${pl.netProfit >= 0 ? 'text-success' : 'text-danger'}">
            $${pl.netProfit.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
          </span>
        </div>
      </div>
  ...
  document.getElementById("btn-quick-payment").addEventListener("click", () => {
    ...
    setTimeout(() => {
      // Find payment voucher sub-tab or trigger modal directly
      const payTab = document.querySelector('.vtab-btn[data-vtab="payment"]');
      if (payTab) payTab.click();
      setTimeout(() => {
        const payBtn = document.getElementById("btn-add-payment");
  ...
  document.getElementById("btn-goto-ledger").addEventListener("click", () => {
    window.location.hash = "#vouchers";
    setTimeout(() => {
      const jvTab = document.querySelector('.vtab-btn[data-vtab="journal"]');
      if (jvTab) jvTab.click();
    }, 50);
  });
}
