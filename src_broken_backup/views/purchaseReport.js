...
export function showPurchaseReportModal(container) {
  ...
  root.innerHTML = `
    <div class="modal-overlay active" id="purchase-report-overlay" style="display:flex; justify-content:center; align-items:center; background: rgba(15,23,42,0.35); backdrop-filter: blur(1px); z-index:2000;">
  ...
  const overlay = document.getElementById("purchase-report-overlay");
    ...
    filteredInvoices.forEach(inv => {
      ...
      if (inv.adjustmentsList && inv.adjustmentsList.length > 0) {
          inv.adjustmentsList.forEach(a => {
              ...
              if (aAmt !== 0) {
                  if (!groupedData[inv.id]) {
                    groupedData[inv.id] = { billNo: inv.id, billDate: inv.date, vendor: inv.contactName, purchaseMan: inv.purchaseMan || '', items: [], totalTax: 0, totalAmt: 0, totalMargin: 0 };
                  }
                  groupedData[inv.id].items.push({
                      product: aName,
                      model: "",
...
export function showProductWisePurchaseReportModal(container) {
  ...
  btnView.addEventListener("click", () => {
    ...
    const fPurchaseman = overlayEl.querySelector("#prod-pur-rep-purchaseman").value;
    const fGroup = overlayEl.querySelector("#prod-pur-rep-prod").value;
