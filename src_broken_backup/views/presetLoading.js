...
export function renderPresetLoading(container) {
  ...
  container.innerHTML = `
    <div style="padding: 1rem; display: flex; flex-direction: column; gap: 1.5rem;">
      <!-- Title Header -->
      <div style="display: flex; align-items: center; justify-content: space-between; border-bottom: 2px solid var(--border-color, #e2e8f0); padding-bottom: 0.5rem;">
        <h2 style="margin: 0; font-size: 1.4rem; font-weight: 700; color: #1e3a8a; display: flex; align-items: center; gap: 8px;">
          ...
          Preset Loading Charges Configuration
        </h2>
        <span style="font-size: 0.8rem; color: #64748b; font-weight: 500;">Configure standard loading charge per unit for each product</span>
      </div>
      ...
      <div style="display: grid; grid-template-columns: 1fr 2fr; gap: 1.5rem; align-items: start;">
        ...
        <div class="panel" style="padding: 1.25rem; background: #f8fafc; border: 1px solid #cbd5e1; border-radius: 6px; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05);">
          <h3 style="margin: 0 0 1rem 0; font-size: 1rem; font-weight: 700; color: #334155; border-bottom: 1px solid #e2e8f0; padding-bottom: 0.5rem;">Set Loading Charge</h3>
          ...
          <form id="preset-loading-form" style="display: flex; flex-direction: column; gap: 1rem;">
            ...
            <div>
              <label for="pl-loading-charge" style="font-weight: 600; display: block; margin-bottom: 4px; font-size: 0.8rem; color: #475569;">Loading Charge / Unit *</label>
            ...
            <table class="data-table">
              <thead>
                <tr style="position: sticky; top: 0; background: var(--bg-surface, #f8fafc); z-index: 1;">
                  ...
                  <th>Code / Model</th>
                  <th>Unit</th>
                  <th style="text-align: right;">Loading Charge / Unit</th>
                  <th style="text-align: center; width: 80px;">Action</th>
                </tr>
              </thead>
              <tbody>
                ${materials.length === 0 ? `
                  <tr><td colspan="5" style="text-align: center; color: var(--text-muted, #64748b); padding: 2rem;">No products found in Master. Create products first.</td></tr>
                ` : materials.map(m => `
  ...
  form.addEventListener("submit", (e) => {
    ...
    if (mat) {
      ...
      mat.loadingChargeEnabled = chargeVal > 0;  // auto-enable when charge set, disable when cleared to 0
      state.saveState();
      alert(`Successfully saved loading charge for ${selectedName} (${selectedCode}) to ₹${chargeVal.toFixed(2)}.`);
