...
export function showSalesReportModal(container) {
  ...
  root.innerHTML = `
    <div class="modal-overlay active" id="sales-report-overlay" style="display:flex; justify-content:center; align-items:center; background: rgba(15,23,42,0.35); backdrop-filter: blur(1px); z-index:2000;">
      <div class="modal-container modal-lg" style="max-width:1400px; width: 96vw; height:90vh; background-color:#cbd5e1; color:#0f172a; padding:10px; font-family: sans-serif; border: 2px solid #5a7b9c; border-radius: 4px; box-shadow: 0 10px 40px rgba(0,0,0,0.4); font-size:0.8rem; display:flex; flex-direction:column; gap:8px;">
        ...
        <div style="background: linear-gradient(180deg, #1e3a8a 0%, #3b82f6 100%); color:white; padding:4px 8px; font-weight:700; display:flex; justify-content:space-between; align-items:center; border-radius: 2px;">
          <div style="display:flex; align-items:center; gap:6px;"><i class="fa-solid fa-file-invoice"></i> SALES REPORT</div>
  ...
  const overlay = document.getElementById("sales-report-overlay");
  ...
  function formatDate(dStr) {
    ...
    if (parts.length === 3) {
      ...
      const d = parts[2];
      return `${d}-${months[m]}-${y.substring(2)}`;
    }
    return dStr;
  }
        ...
        <div style="background:#b4c6e7; padding:6px; border:1px solid #8faadc; border-radius:2px; display:flex; flex-direction:column; gap:6px;">
          ...
          <div style="display:flex; flex-wrap:wrap; gap:8px; align-items:center;">
            ...
            <div style="display:flex; align-items:center; gap:4px; flex-grow:1; min-width:240px;">
              ...
              <select id="margin-customer-select" class="form-control" style="padding:2px; font-size:0.72rem; background:white; color:black; height:22px; width:100%; max-width:260px;">
                <option value="All">All</option>
...
export function showCategoryWiseSalesReportModal(container) {
  ...
  tempDiv.innerHTML = `
    <div class="modal-overlay active" id="category-wise-sales-report-overlay" style="display:flex; justify-content:center; align-items:center; background: rgba(15,23,42,0.35); backdrop-filter: blur(1px); z-index:2000;">
      <style>
        .cat-rep-hide-gst .col-gst { display: none !important; }
        .cat-rep-hide-margin .col-margin { display: none !important; }
      </style>
      <div class="modal-container modal-lg" style="width: 80vw; height:80vh; max-width:1400px; background-color:#dbeafe; color:#0f172a; padding:8px; font-family: sans-serif; border: 2px solid #5a7b9c; border-radius: 4px; box-shadow: 0 10px 40px rgba(0,0,0,0.4); font-size:0.8rem; display:flex; flex-direction:column; gap:8px;">
        ...
        <!-- Header Ribbon -->
        <div style="background: linear-gradient(180deg, #1e3a8a 0%, #3b82f6 100%); color:white; padding:4px 8px; font-weight:700; display:flex; justify-content:space-between; align-items:center; border-radius: 2px;">
          <div style="display:flex; align-items:center; gap:6px;"><i class="fa-solid fa-chart-bar"></i> CATEGORY WISE SALES REPORT</div>
          <button type="button" class="win-btn" style="background:none; border:none; color:white; font-size:1rem; cursor:pointer;" id="cat-rep-close-icon">&times;</button>
        </div>
        ...
        <div style="display:flex; flex-wrap:wrap; gap:8px; align-items:flex-end; background:#b4c6e7; padding:8px; border:1px solid #8faadc; border-radius:2px;">
          ...
          <div style="display:flex; flex-direction:column; gap:2px;">
            ...
            <input type="date" id="cat-rep-date-from" class="form-control" style="width:125px; padding:2px; font-size:0.75rem;" value="${firstDay}">
          </div>

          <div style="display:flex; flex-direction:column; gap:2px;">
            <label style="font-weight:bold; font-size:0.75rem;">To:</label>
            <input type="date" id="cat-rep-date-to" class="form-control" style="width:125px; padding:2px; font-size:0.75rem;" value="${lastDay}">
          </div>

          <div style="display:flex; flex-direction:column; gap:2px;">
            <label style="font-weight:bold; font-size:0.75rem;">Category Name:</label>
            <select id="cat-rep-cat" class="form-control" style="width:140px; padding:2px; font-size:0.75rem;">
              <option value="All">All</option>
          ...
          <div style="display:flex; flex-direction:column; gap:2px;">
            ...
            <select id="cat-rep-subcat" class="form-control" style="width:140px; padding:2px; font-size:0.75rem;">
              <option value="All">All</option>
              ${subCategories.map(sc => `<option value="${sc}">${sc}</option>`).join("")}
            </select>
          </div>
          ...
          <div style="display:flex; flex-direction:column; gap:2px;">
            ...
            <select id="cat-rep-tax" class="form-control" style="width:80px; padding:2px; font-size:0.75rem;">
              <option value="All">All</option>
          ...
          <div style="display:flex; flex-direction:column; gap:2px;">
            <label style="font-weight:bold; font-size:0.75rem;">Customer Name:</label>
            <select id="cat-rep-cust" class="form-control" style="width:180px; padding:2px; font-size:0.75rem;">
              <option value="All">All</option>
          ...
          <div style="display:flex; flex-direction:column; gap:4px; margin-left:10px; justify-content:center; padding-bottom: 2px;">
            <label style="font-size:0.75rem; font-weight:bold; display:flex; align-items:center; gap:4px; cursor:pointer;">
              <input type="checkbox" id="cat-rep-hide-gst"> Hide GST Column
            </label>
            <label style="font-size:0.75rem; font-weight:bold; display:flex; align-items:center; gap:4px; cursor:pointer;">
              <input type="checkbox" id="cat-rep-hide-margin"> Hide Margin Column
            </label>
          </div>

          <div style="font-weight:bold; font-size:1rem; color:blue; margin-left:auto; margin-bottom:4px;" id="cat-rep-total-amt">
            Total Amt: 0.00
          </div>

          <div style="display:flex; gap:6px; margin-bottom:2px;">
            <button type="button" class="btn btn-secondary" id="btn-cat-rep-view" style="padding:2px 15px; font-weight:bold; background-color:#e2e8f0; border:1px solid #475569; color:black; height:24px; box-shadow:1px 1px 2px white inset;">View</button>
            <button type="button" class="btn btn-secondary" id="btn-cat-rep-print" style="padding:2px 15px; font-weight:bold; background-color:#e2e8f0; border:1px solid #475569; color:black; height:24px; box-shadow:1px 1px 2px white inset;">Print</button>
            <button type="button" class="btn btn-secondary" id="btn-cat-rep-close" style="padding:2px 15px; font-weight:bold; background-color:#e2e8f0; border:1px solid #475569; color:black; height:24px; box-shadow:1px 1px 2px white inset;">Close</button>
          </div>
        </div>
        ...
        <div style="flex-grow:1; background:white; border:1px solid #94a3b8; overflow-y:auto; overflow-x:auto; border-radius:2px;" id="print-area-cat-rep">
          <table style="width:100%; border-collapse:collapse; text-align:left; font-size:0.8rem; color:black; white-space:nowrap;" id="cat-rep-table">
            <thead>
              <tr style="background-color:#1e3b8b; color:white; font-weight:bold; position:sticky; top:0;">
                <th style="padding:6px 4px; border:1px solid #cbd5e1;">Bill No</th>
                <th style="padding:6px 4px; border:1px solid #cbd5e1;">Bill Date</th>
                <th style="padding:6px 4px; border:1px solid #cbd5e1;">Customer</th>
                ...
                <th style="padding:6px 4px; border:1px solid #cbd5e1; text-align:right;">Adjustment</th>
                <th style="padding:6px 4px; border:1px solid #cbd5e1; text-align:right;">Total</th>
                <th class="col-margin" style="padding:6px 4px; border:1px solid #cbd5e1; text-align:right;">Margin</th>
              </tr>
            </thead>
            <tbody id="cat-rep-tbody">
              <!-- Rendered rows -->
            </tbody>
          </table>
        </div>
  ...
  const overlay = document.getElementById("category-wise-sales-report-overlay");
  const close = () => { overlay.remove(); };

  overlayEl.querySelector("#cat-rep-close-icon").addEventListener("click", close);
  overlayEl.querySelector("#btn-cat-rep-close").addEventListener("click", close);

  // Toggle column visibility
  overlayEl.querySelector("#cat-rep-hide-gst").addEventListener("change", (e) => {
    const tbl = overlayEl.querySelector("#cat-rep-table");
    if (e.target.checked) tbl.classList.add("cat-rep-hide-gst");
    else tbl.classList.remove("cat-rep-hide-gst");
  });
  
  overlayEl.querySelector("#cat-rep-hide-margin").addEventListener("change", (e) => {
    const tbl = overlayEl.querySelector("#cat-rep-table");
    if (e.target.checked) tbl.classList.add("cat-rep-hide-margin");
    else tbl.classList.remove("cat-rep-hide-margin");
  });

  overlayEl.querySelector("#btn-cat-rep-print").addEventListener("click", () => {
    const printContent = overlayEl.querySelector("#print-area-cat-rep").innerHTML;
    const hideGst = overlayEl.querySelector("#cat-rep-hide-gst").checked;
    const hideMargin = overlayEl.querySelector("#cat-rep-hide-margin").checked;
  ...
  const btnView = overlayEl.querySelector("#btn-cat-rep-view");
  const tbody = overlayEl.querySelector("#cat-rep-tbody");
  const lblTotalAmt = overlayEl.querySelector("#cat-rep-total-amt");

  btnView.addEventListener("click", () => {
    const fFrom = overlayEl.querySelector("#cat-rep-date-from").value;
    const fTo = overlayEl.querySelector("#cat-rep-date-to").value;
    const fCat = overlayEl.querySelector("#cat-rep-cat").value;
    const fSubCat = overlayEl.querySelector("#cat-rep-subcat").value;
    const fTax = overlayEl.querySelector("#cat-rep-tax").value;
    const fCust = overlayEl.querySelector("#cat-rep-cust").value;
    ...
    filteredInvoices.forEach(inv => {
      ...
      inv.items.forEach((item, index) => {
        ...
        // Exclusive sales amount (after discount, before tax)
        ...
        // Show bill-level adjustment ONLY on the first item row for this bill
        const adjVal = (index === 0) ? billAdj : 0;
        
        // Total includes adjustment
        const finalItemTotal = inclusiveAmt + adjVal;
        ...
        groupedData[inv.id].totalAdj += adjVal;
    ...
    Object.values(groupedData).forEach((group, idx) => {
        ...
        group.items.forEach((it, i) => {
            html += `
            <tr style="background:${bg}; cursor:pointer;" class="cat-rep-row" data-billno="${group.billNo}">
              ...
              <td style="padding:4px; border:1px solid #cbd5e1;">${i === 0 ? group.customer : ''}</td>
              ...
              <td style="padding:4px; border:1px solid #cbd5e1; text-align:right;">${i === 0 ? it.adj.toFixed(2) : ''}</td>
              <td style="padding:4px; border:1px solid #cbd5e1; text-align:right;">${it.total}</td>
              <td class="col-margin" style="padding:4px; border:1px solid #cbd5e1; text-align:right;">${it.margin}</td>
            </tr>
            `;
        });
        ...
        <tr style="background:#e2e8f0; font-weight:bold;">
          <td colspan="7" style="padding:4px; border:1px solid #cbd5e1; text-align:right;">Total</td>
          <td class="col-gst" style="padding:4px; border:1px solid #cbd5e1; text-align:right; color:#166534;">${group.totalTax.toFixed(2)}</td>
          <td style="padding:4px; border:1px solid #cbd5e1; text-align:right;">${group.totalAdj.toFixed(2)}</td>
          <td style="padding:4px; border:1px solid #cbd5e1; text-align:right; color:#1e3b8b;">${group.totalAmt.toFixed(2)}</td>
          <td class="col-margin" style="padding:4px; border:1px solid #cbd5e1; text-align:right; color:#991b1b;">${group.totalMargin.toFixed(2)}</td>
        </tr>
        `;
    });
    ...
    <tr style="background:#cbd5e1; font-weight:bold; font-size:0.9rem;">
      ...
      <td style="padding:6px 4px; border:1px solid #94a3b8; text-align:right;">${gTotalAdj.toFixed(2)}</td>
      <td style="padding:6px 4px; border:1px solid #94a3b8; text-align:right; color:#1e3b8b;">${gTotalAmount.toFixed(2)}</td>
      <td class="col-margin" style="padding:6px 4px; border:1px solid #94a3b8; text-align:right; color:#991b1b;">${gTotalMargin.toFixed(2)}</td>
    </tr>
    `;
    ...
    overlayEl.querySelectorAll(".cat-rep-row").forEach(row => {
      row.addEventListener("dblclick", () => {
        const bNo = row.getAttribute("data-billno");
        const invoice = invoices.find(inv => String(inv.id) === String(bNo));
        if (invoice) {
           import("./transactions.js").then(m => {
             m.showInvoiceBuilderModal(container, customers, materials, () => {}, invoice);
           });
        }
      });
    });
...
export function showCustomerWiseSalesReportModal(container) {
  const root = document.getElementById("modal-container-root") || container;
  const materials = state.getMaterials();
  const invoices = state.getInvoices();
  const contacts = state.getContacts();
  const customers = contacts.filter(c => c.type === "customer" || c.listInCustomerList === true);
  const salesMen = [...new Set(invoices.map(i => i.salesMan).filter(Boolean))];
  ...
  const tempDiv = document.createElement("div");
  tempDiv.innerHTML = `
    <div class="modal-overlay active" id="customer-wise-sales-report-overlay" style="display:flex; justify-content:center; align-items:center; background: rgba(15,23,42,0.35); backdrop-filter: blur(1px); z-index:2000;">
      <style>
        .cust-rep-hide-gst .col-gst { display: none !important; }
        .cust-rep-hide-margin .col-margin { display: none !important; }
      </style>
      <div class="modal-container modal-lg" style="width: 80vw; height:80vh; max-width:1400px; background-color:#dbeafe; color:#0f172a; padding:8px; font-family: sans-serif; border: 2px solid #5a7b9c; border-radius: 4px; box-shadow: 0 10px 40px rgba(0,0,0,0.4); font-size:0.8rem; display:flex; flex-direction:column; gap:8px;">
        
        <!-- Header Ribbon -->
        <div style="background: linear-gradient(180deg, #1e3a8a 0%, #3b82f6 100%); color:white; padding:4px 8px; font-weight:700; display:flex; justify-content:space-between; align-items:center; border-radius: 2px;">
          <div style="display:flex; align-items:center; gap:6px;"><i class="fa-solid fa-chart-bar"></i> CUSTOMER WISE SALES REPORT</div>
          <button type="button" class="win-btn" style="background:none; border:none; color:white; font-size:1rem; cursor:pointer;" id="cust-rep-close-icon">&times;</button>
        </div>
        ...
        <div style="display:flex; flex-wrap:wrap; gap:8px; align-items:flex-end; background:#b4c6e7; padding:8px; border:1px solid #8faadc; border-radius:2px;">
          ...
          <div style="display:flex; flex-direction:column; gap:2px;">
            <label style="font-weight:bold; font-size:0.75rem;">From:</label>
            <input type="date" id="cust-rep-date-from" class="form-control" style="width:125px; padding:2px; font-size:0.75rem;" value="${firstDay}">
          </div>

          <div style="display:flex; flex-direction:column; gap:2px;">
            <label style="font-weight:bold; font-size:0.75rem;">To:</label>
            <input type="date" id="cust-rep-date-to" class="form-control" style="width:125px; padding:2px; font-size:0.75rem;" value="${lastDay}">
          </div>

          <div style="display:flex; flex-direction:column; gap:2px;">
            <label style="font-weight:bold; font-size:0.75rem;">Customer Name:</label>
            <select id="cust-rep-cust" class="form-control" style="width:180px; padding:2px; font-size:0.75rem;">
              <option value="All">All</option>
              ${customers.map(c => `<option value="${c.name}">${c.name}</option>`).join("")}
            </select>
          </div>
          ...
          <div style="display:flex; flex-direction:column; gap:2px;">
            <label style="font-weight:bold; font-size:0.75rem;">Sales Man:</label>
            <select id="cust-rep-salesman" class="form-control" style="width:140px; padding:2px; font-size:0.75rem;">
              <option value="All">All</option>
              ${salesMen.map(s => `<option value="${s}">${s}</option>`).join("")}
            </select>
          </div>

          <div style="display:flex; flex-direction:column; gap:4px; margin-left:10px; justify-content:center; padding-bottom: 2px;">
            <label style="font-size:0.75rem; font-weight:bold; display:flex; align-items:center; gap:4px; cursor:pointer;">
              <input type="checkbox" id="cust-rep-hide-gst"> Hide GST Column
            </label>
            <label style="font-size:0.75rem; font-weight:bold; display:flex; align-items:center; gap:4px; cursor:pointer;">
              <input type="checkbox" id="cust-rep-hide-margin"> Hide Margin Column
            </label>
          </div>

          <div style="font-weight:bold; font-size:1rem; color:blue; margin-left:auto; margin-bottom:4px;" id="cust-rep-total-amt">
            Total Amt: 0.00
          </div>

          <div style="display:flex; gap:6px; margin-bottom:2px;">
            <button type="button" class="btn btn-secondary" id="btn-cust-rep-view" style="padding:2px 15px; font-weight:bold; background-color:#e2e8f0; border:1px solid #475569; color:black; height:24px; box-shadow:1px 1px 2px white inset;">View</button>
            <button type="button" class="btn btn-secondary" id="btn-cust-rep-print" style="padding:2px 15px; font-weight:bold; background-color:#e2e8f0; border:1px solid #475569; color:black; height:24px; box-shadow:1px 1px 2px white inset;">Print</button>
            <button type="button" class="btn btn-secondary" id="btn-cust-rep-close" style="padding:2px 15px; font-weight:bold; background-color:#e2e8f0; border:1px solid #475569; color:black; height:24px; box-shadow:1px 1px 2px white inset;">Close</button>
          </div>
        </div>
        ...
        <div style="flex-grow:1; background:white; border:1px solid #94a3b8; overflow-y:auto; overflow-x:auto; border-radius:2px;" id="print-area-cust-rep">
          <table style="width:100%; border-collapse:collapse; text-align:left; font-size:0.8rem; color:black; white-space:nowrap;" id="cust-rep-table">
            <thead>
              <tr style="background-color:#1e3b8b; color:white; font-weight:bold; position:sticky; top:0;">
                <th style="padding:6px 4px; border:1px solid #cbd5e1;">Bill No</th>
                <th style="padding:6px 4px; border:1px solid #cbd5e1;">Bill Date</th>
                ...
                <th style="padding:6px 4px; border:1px solid #cbd5e1; text-align:right;">Total</th>
                <th class="col-margin" style="padding:6px 4px; border:1px solid #cbd5e1; text-align:right;">Margin</th>
                <th style="padding:6px 4px; border:1px solid #cbd5e1;">Sales Man</th>
              </tr>
            </thead>
            <tbody id="cust-rep-tbody">
              <!-- Rendered rows -->
            </tbody>
          </table>
        </div>
  ...
  const overlay = document.getElementById("customer-wise-sales-report-overlay");
  const close = () => { overlay.remove(); };

  overlayEl.querySelector("#cust-rep-close-icon").addEventListener("click", close);
  overlayEl.querySelector("#btn-cust-rep-close").addEventListener("click", close);

  // Toggle column visibility
  overlayEl.querySelector("#cust-rep-hide-gst").addEventListener("change", (e) => {
    const tbl = overlayEl.querySelector("#cust-rep-table");
    if (e.target.checked) tbl.classList.add("cust-rep-hide-gst");
    else tbl.classList.remove("cust-rep-hide-gst");
  });
  
  overlayEl.querySelector("#cust-rep-hide-margin").addEventListener("change", (e) => {
    const tbl = overlayEl.querySelector("#cust-rep-table");
    if (e.target.checked) tbl.classList.add("cust-rep-hide-margin");
    else tbl.classList.remove("cust-rep-hide-margin");
  });

  overlayEl.querySelector("#btn-cust-rep-print").addEventListener("click", () => {
    const printContent = overlayEl.querySelector("#print-area-cust-rep").innerHTML;
    const hideGst = overlayEl.querySelector("#cust-rep-hide-gst").checked;
    const hideMargin = overlayEl.querySelector("#cust-rep-hide-margin").checked;
  ...
  const btnView = overlayEl.querySelector("#btn-cust-rep-view");
  const tbody = overlayEl.querySelector("#cust-rep-tbody");
  const lblTotalAmt = overlayEl.querySelector("#cust-rep-total-amt");

  btnView.addEventListener("click", () => {
    const fFrom = overlayEl.querySelector("#cust-rep-date-from").value;
    const fTo = overlayEl.querySelector("#cust-rep-date-to").value;
    const fCust = overlayEl.querySelector("#cust-rep-cust").value;
    const fSalesman = overlayEl.querySelector("#cust-rep-salesman").value;
    ...
    filteredInvoices.forEach(inv => {
      // Check customer filter
      if (fCust !== "All" && inv.contactName !== fCust) return;
      if (fSalesman !== "All" && inv.salesMan !== fSalesman) return;
      ...
      if (inv.adjustmentsList && inv.adjustmentsList.length > 0) {
          inv.adjustmentsList.forEach(a => {
              const aName = a.name || a.account || "Adjustment";
              const aAmt = (a.type === "Add" ? 1 : -1) * (parseFloat(a.amount) || 0);
              if (aAmt !== 0) {
                  if (!groupedData[inv.id]) {
                    groupedData[inv.id] = { billNo: inv.id, billDate: inv.date, customer: inv.contactName, salesMan: inv.salesMan || '', items: [], totalTax: 0, totalAmt: 0, totalMargin: 0 };
                  }
                  groupedData[inv.id].items.push({
                      product: aName,
                      model: "",
                      ...
                      discount: "",
                      tax: "",
                      total: aAmt.toFixed(2),
                      margin: ""
                  });
                  groupedData[inv.id].totalAmt += aAmt;
                  gTotalAmount += aAmt;
              }
          });
      } else if (parseFloat(inv.shipping) || parseFloat(inv.adjustments)) {
          let aAmt = (parseFloat(inv.shipping) || 0) + (parseFloat(inv.adjustments) || 0);
          if (aAmt !== 0) {
              if (!groupedData[inv.id]) {
                groupedData[inv.id] = { billNo: inv.id, billDate: inv.date, customer: inv.contactName, salesMan: inv.salesMan || '', items: [], totalTax: 0, totalAmt: 0, totalMargin: 0 };
              }
              groupedData[inv.id].items.push({
                  product: "Shipping / Adjustments",
                  model: "",
                  qty: "",
                  rate: "",
                  discount: "",
    ...
    Object.values(groupedData).forEach((group, idx) => {
        ...
        group.items.forEach((it, i) => {
            html += `
            <tr style="background:${bg}; cursor:pointer;" class="cust-rep-row" data-billno="${group.billNo}">
              <td style="padding:4px; border:1px solid #cbd5e1;">${i === 0 ? group.billNo : ''}</td>
              <td style="padding:4px; border:1px solid #cbd5e1;">${i === 0 ? group.billDate : ''}</td>
              <td style="padding:4px; border:1px solid #cbd5e1;">${i === 0 ? group.customer : ''}</td>
              <td style="padding:4px; border:1px solid #cbd5e1;">${it.product}</td>
              ...
              <td style="padding:4px; border:1px solid #cbd5e1; text-align:right;">${it.total}</td>
              <td class="col-margin" style="padding:4px; border:1px solid #cbd5e1; text-align:right;">${it.margin}</td>
              <td style="padding:4px; border:1px solid #cbd5e1;">${i === 0 ? group.salesMan : ''}</td>
            </tr>
            `;
        });
    ...
    // Attach double click to open invoice
    overlayEl.querySelectorAll(".cust-rep-row").forEach(row => {
      row.addEventListener("dblclick", () => {
        const bNo = row.getAttribute("data-billno");
        const invoice = invoices.find(inv => String(inv.id) === String(bNo));
        if (invoice) {
...
export function showProductGroupWiseSalesReportModal(container) {
  const root = document.getElementById("modal-container-root") || container;
  const materials = state.getMaterials();
  const invoices = state.getInvoices();
  const contacts = state.getContacts();
  ...
  const tempDiv = document.createElement("div");
  tempDiv.innerHTML = `
    <div class="modal-overlay active" id="product-group-wise-sales-report-overlay" style="display:flex; justify-content:center; align-items:center; background: rgba(15,23,42,0.35); backdrop-filter: blur(1px); z-index:2000;">
      <style>
        .grp-rep-hide-gst .col-gst { display: none !important; }
        .grp-rep-hide-margin .col-margin { display: none !important; }
      </style>
      <div class="modal-container modal-lg" style="width: 80vw; height:80vh; max-width:1400px; background-color:#dbeafe; color:#0f172a; padding:8px; font-family: sans-serif; border: 2px solid #5a7b9c; border-radius: 4px; box-shadow: 0 10px 40px rgba(0,0,0,0.4); font-size:0.8rem; display:flex; flex-direction:column; gap:8px;">
        
        <!-- Header Ribbon -->
        <div style="background: linear-gradient(180deg, #1e3a8a 0%, #3b82f6 100%); color:white; padding:4px 8px; font-weight:700; display:flex; justify-content:space-between; align-items:center; border-radius: 2px;">
          <div style="display:flex; align-items:center; gap:6px;"><i class="fa-solid fa-chart-bar"></i> PRODUCT GROUP WISE SALES REPORT</div>
          <button type="button" class="win-btn" style="background:none; border:none; color:white; font-size:1rem; cursor:pointer;" id="grp-rep-close-icon">&times;</button>
        </div>
        ...
        <div style="display:flex; flex-wrap:wrap; gap:8px; align-items:flex-end; background:#b4c6e7; padding:8px; border:1px solid #8faadc; border-radius:2px;">
          ...
          <div style="display:flex; flex-direction:column; gap:2px;">
            <label style="font-weight:bold; font-size:0.75rem;">From:</label>
            <input type="date" id="grp-rep-date-from" class="form-control" style="width:125px; padding:2px; font-size:0.75rem;" value="${firstDay}">
          </div>

          <div style="display:flex; flex-direction:column; gap:2px;">
            <label style="font-weight:bold; font-size:0.75rem;">To:</label>
            <input type="date" id="grp-rep-date-to" class="form-control" style="width:125px; padding:2px; font-size:0.75rem;" value="${lastDay}">
          </div>

          <div style="display:flex; flex-direction:column; gap:2px;">
            <label style="font-weight:bold; font-size:0.75rem;">Product Group:</label>
            <select id="grp-rep-group" class="form-control" style="width:140px; padding:2px; font-size:0.75rem;">
              <option value="All">All</option>
          ...
          <div style="display:flex; flex-direction:column; gap:2px;">
            <label style="font-weight:bold; font-size:0.75rem;">Customer Name:</label>
            <select id="grp-rep-cust" class="form-control" style="width:180px; padding:2px; font-size:0.75rem;">
              <option value="All">All</option>
              ${customers.map(c => `<option value="${c.name}">${c.name}</option>`).join("")}
            </select>
          </div>
          ...
          <div style="display:flex; flex-direction:column; gap:2px;">
            <label style="font-weight:bold; font-size:0.75rem;">Sales Man:</label>
            <select id="grp-rep-salesman" class="form-control" style="width:140px; padding:2px; font-size:0.75rem;">
              <option value="All">All</option>
              ${salesMen.map(s => `<option value="${s}">${s}</option>`).join("")}
            </select>
          </div>
          ...
          <div style="display:flex; flex-direction:column; gap:4px; margin-left:10px; justify-content:center; padding-bottom: 2px;">
            <label style="font-size:0.75rem; font-weight:bold; display:flex; align-items:center; gap:4px; cursor:pointer;">
              <input type="checkbox" id="grp-rep-hide-gst"> Hide GST Column
            </label>
            <label style="font-size:0.75rem; font-weight:bold; display:flex; align-items:center; gap:4px; cursor:pointer;">
              <input type="checkbox" id="grp-rep-hide-margin"> Hide Margin Column
            </label>
          </div>

          <div style="font-weight:bold; font-size:1rem; color:blue; margin-left:auto; margin-bottom:4px;" id="grp-rep-total-amt">
            Total Amt: 0.00
          </div>

          <div style="display:flex; gap:6px; margin-bottom:2px;">
            <button type="button" class="btn btn-secondary" id="btn-grp-rep-view" style="padding:2px 15px; font-weight:bold; background-color:#e2e8f0; border:1px solid #475569; color:black; height:24px; box-shadow:1px 1px 2px white inset;">View</button>
            <button type="button" class="btn btn-secondary" id="btn-grp-rep-print" style="padding:2px 15px; font-weight:bold; background-color:#e2e8f0; border:1px solid #475569; color:black; height:24px; box-shadow:1px 1px 2px white inset;">Print</button>
            <button type="button" class="btn btn-secondary" id="btn-grp-rep-close" style="padding:2px 15px; font-weight:bold; background-color:#e2e8f0; border:1px solid #475569; color:black; height:24px; box-shadow:1px 1px 2px white inset;">Close</button>
          </div>
        </div>

        <!-- Table View -->
        <div style="flex-grow:1; background:white; border:1px solid #94a3b8; overflow-y:auto; overflow-x:auto; border-radius:2px;" id="print-area-cust-rep">
          <table style="width:100%; border-collapse:collapse; text-align:left; font-size:0.8rem; color:black; white-space:nowrap;" id="grp-rep-table">
            <thead>
              <tr style="background-color:#1e3b8b; color:white; font-weight:bold; position:sticky; top:0;">
                <th style="padding:6px 4px; border:1px solid #cbd5e1;">Bill No</th>
                <th style="padding:6px 4px; border:1px solid #cbd5e1;">Bill Date</th>
  ...
  const overlay = document.getElementById("product-group-wise-sales-report-overlay");
  const close = () => { overlay.remove(); };

  overlayEl.querySelector("#grp-rep-close-icon").addEventListener("click", close);
  overlayEl.querySelector("#btn-grp-rep-close").addEventListener("click", close);

  // Toggle column visibility
  overlayEl.querySelector("#grp-rep-hide-gst").addEventListener("change", (e) => {
    const tbl = overlayEl.querySelector("#grp-rep-table");
    if (e.target.checked) tbl.classList.add("grp-rep-hide-gst");
    else tbl.classList.remove("grp-rep-hide-gst");
  });
  
  overlayEl.querySelector("#grp-rep-hide-margin").addEventListener("change", (e) => {
    const tbl = overlayEl.querySelector("#grp-rep-table");
    if (e.target.checked) tbl.classList.add("grp-rep-hide-margin");
    else tbl.classList.remove("grp-rep-hide-margin");
  });

  overlayEl.querySelector("#btn-grp-rep-print").addEventListener("click", () => {
    const printContent = overlayEl.querySelector("#print-area-cust-rep").innerHTML;
    const hideGst = overlayEl.querySelector("#grp-rep-hide-gst").checked;
    const hideMargin = overlayEl.querySelector("#grp-rep-hide-margin").checked;

    const printWindow = window.open("", "_blank");
    printWindow.document.write(`
      <html>
        <head>
          <title>PRODUCT GROUP WISE SALES REPORT</title>
          <style>
            body { font-family: sans-serif; font-size: 0.8rem; margin: 20px; }
            table { width: 100%; border-collapse: collapse; }
            th, td { border: 1px solid #000; padding: 4px; }
  ...
  const btnView = overlayEl.querySelector("#btn-grp-rep-view");
  const tbody = overlayEl.querySelector("#grp-rep-tbody");
  const lblTotalAmt = overlayEl.querySelector("#grp-rep-total-amt");

  btnView.addEventListener("click", () => {
    const fFrom = overlayEl.querySelector("#grp-rep-date-from").value;
    const fTo = overlayEl.querySelector("#grp-rep-date-to").value;
    const fCust = overlayEl.querySelector("#grp-rep-cust").value;
    const fSalesman = overlayEl.querySelector("#grp-rep-salesman").value;
    const fGroup = overlayEl.querySelector("#grp-rep-group").value;
    ...
    Object.values(groupedData).forEach((group, idx) => {
        ...
        group.items.forEach((it, i) => {
            html += `
            <tr style="background:${bg}; cursor:pointer;" class="grp-rep-row" data-billno="${group.billNo}">
              <td style="padding:4px; border:1px solid #cbd5e1;">${i === 0 ? group.billNo : ''}</td>
              <td style="padding:4px; border:1px solid #cbd5e1;">${i === 0 ? group.billDate : ''}</td>
              <td style="padding:4px; border:1px solid #cbd5e1;">${i === 0 ? group.customer : ''}</td>
              <td style="padding:4px; border:1px solid #cbd5e1;">${it.product}</td>
    ...
    // Attach double click to open invoice
    overlayEl.querySelectorAll(".grp-rep-row").forEach(row => {
      row.addEventListener("dblclick", () => {
        const bNo = row.getAttribute("data-billno");
        const invoice = invoices.find(inv => String(inv.id) === String(bNo));
        if (invoice) {
