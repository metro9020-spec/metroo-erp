...
class StateManager {
  ...
  getProfitLoss(startDate = "", endDate = "") {
    ...
    const getGroupCategory = (gName) => {
      if (!gName) return "";
      let currentGroup = (this.accountGroups || []).find(g => String(g.name || "").toUpperCase() === gName.toUpperCase());
      if (!currentGroup) {
      ...
      for (let i = 0; i < 10; i++) {
        ...
        const parentGroup = (this.accountGroups || []).find(g => String(g.name || "").toUpperCase() === under);
      ...
      if (nameUpper === "LIABILITIES" || nameUpper.includes("LIABILITY") || nameUpper === "EQUITY" || nameUpper.includes("CAPITAL") || nameUpper.includes("CREDITORS") || nameUpper.includes("TAX") || nameUpper.includes("DUTIES") || nameUpper.includes("GST") || nameUpper.includes("CESS")) return "BALANCE_SHEET";
      if (nameUpper.includes("EXPENSE") || nameUpper.includes("COST") || nameUpper.includes("LOSS") || nameUpper === "DEPRECIATION") return "EXPENSE";
      if (nameUpper.includes("INCOME") || nameUpper.includes("REVENUE") || nameUpper.includes("GAIN")) return "INCOME";
      return "";
    };
  ...
  getBalanceSheet(startDate = "", endDate = "") {
    ...
    let capitalVal = balances["CAPITAL / OWNER EQUITY"] ? -balances["CAPITAL / OWNER EQUITY"].balance : 0;
    let currentLiabilitiesVal = (balances["SUNDRY CREDITORS"] ? -balances["SUNDRY CREDITORS"].balance : 0) + (balances["DUTIES & TAXES"] ? -balances["DUTIES & TAXES"].balance : 0);
    let fixedAssetsVal = 0;
    let currentAssetsVal = (balances["CASH-IN-HAND"] ? balances["CASH-IN-HAND"].balance : 0) + 
                           (balances["BANK CURRENT ACCOUNT"] ? balances["BANK CURRENT ACCOUNT"].balance : 0) + 
                           (balances["SUNDRY DEBTORS"] ? balances["SUNDRY DEBTORS"].balance : 0) + 
                           closingStockVal;
    ...
    if (balances["CAPITAL / OWNER EQUITY"] && balances["CAPITAL / OWNER EQUITY"].balance !== 0) {
      capitalDetails.push({ name: "Capital / Owner Equity", balance: -balances["CAPITAL / OWNER EQUITY"].balance });
    }
    if (balances["SUNDRY CREDITORS"] && balances["SUNDRY CREDITORS"].balance !== 0) {
      currentLiabilitiesDetails.push({ name: "Sundry Creditors (Accounts Payable)", balance: -balances["SUNDRY CREDITORS"].balance });
    }
    if (balances["DUTIES & TAXES"] && balances["DUTIES & TAXES"].balance !== 0) {
      currentLiabilitiesDetails.push({ name: "GST/VAT Payable", balance: -balances["DUTIES & TAXES"].balance });
    }
    if (balances["CASH-IN-HAND"] && balances["CASH-IN-HAND"].balance !== 0) {
      currentAssetsDetails.push({ name: "Cash in Hand", balance: balances["CASH-IN-HAND"].balance });
    }
    if (balances["BANK CURRENT ACCOUNT"] && balances["BANK CURRENT ACCOUNT"].balance !== 0) {
      currentAssetsDetails.push({ name: "Bank Current Account", balance: balances["BANK CURRENT ACCOUNT"].balance });
    }
    if (balances["SUNDRY DEBTORS"] && balances["SUNDRY DEBTORS"].balance !== 0) {
      currentAssetsDetails.push({ name: "Sundry Debtors (Accounts Receivable)", balance: balances["SUNDRY DEBTORS"].balance });
    }
    ...
    const hardcodedAccounts = [
      "CAPITAL / OWNER EQUITY", "SUNDRY CREDITORS", "DUTIES & TAXES",
      "CASH-IN-HAND", "BANK CURRENT ACCOUNT", "SUNDRY DEBTORS"
    ];

    Object.keys(balances).forEach(accId => {
      if (hardcodedAccounts.includes(accId)) return;

      const bal = balances[accId].balance;
      if (bal === 0) return;
      ...
      // Exclude core Sales, Purchase, COGS, Stock hand, and Return accounts
      const details = getAccountDetails(accId);
      const nameUpper = String(details.name || "").toUpperCase();
      const groupUpper = String(details.groupName || "").toUpperCase();
      ...
      if (isCapitalAccount(groupUpper) || nameUpper.includes("CAPITAL") || nameUpper.includes("EQUITY")) {
        capitalVal -= bal;
        capitalDetails.push({ name: details.name, balance: -bal });
      } else if (isLiabilitiesAccount(groupUpper) || nameUpper.includes("LIABILITY") || nameUpper.includes("CREDITORS") || nameUpper.includes("TAX") || nameUpper.includes("DUTIES") || nameUpper.includes("GST") || nameUpper.includes("CESS")) {
        currentLiabilitiesVal -= bal;
        currentLiabilitiesDetails.push({ name: details.name, balance: -bal });
      } else if (isFixedAssetsAccount(groupUpper)) {
        fixedAssetsVal += bal;
        fixedAssetsDetails.push({ name: details.name, balance: bal });
      } else if (isCurrentAssetsAccount(groupUpper) || nameUpper.includes("ASSET") || nameUpper.includes("DEBTORS") || nameUpper.includes("CASH") || nameUpper.includes("BANK") || nameUpper.includes("INVESTMENT")) {
        currentAssetsVal += bal;
        currentAssetsDetails.push({ name: details.name, balance: bal });
      }
    });
