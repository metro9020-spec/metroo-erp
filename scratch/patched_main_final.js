...
salesReportsSubmenuLinks.forEach(link => {
  ...
  if (el) {
    el.addEventListener("click", (e) => {
      e.preventDefault();
      if (link.id === "menu-sales-category") {
        import("./views/salesReport.js").then(m => {
          m.showCategoryWiseSalesReportModal(windowContentEl);
        });
        return;
      }
      if (link.id === "menu-sales-cust-summary") {
        import("./views/salesReport.js").then(m => {
          m.showCustomerWiseSalesReportModal(windowContentEl);
        });
        return;
      }
      if (link.id === "menu-sales-group-wise") {
        import("./views/salesReport.js").then(m => {
          m.showProductGroupWiseSalesReportModal(windowContentEl);
        });
        return;
      }
...
// Purchase Report Submenu Bindings
const mPurchaseReportMain = document.getElementById("menu-purchase-report-main");
if (mPurchaseReportMain) {
  mPurchaseReportMain.addEventListener("click", (e) => {
    e.preventDefault();
    import("./views/purchaseReport.js").then(m => {
      m.showPurchaseReportModal(windowContentEl);
    });
  });
}
