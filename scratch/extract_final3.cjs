...
const erpIdx = content.indexOf("erp-menu-bar");
const start = content.lastIndexOf("<nav", erpIdx);
const end = content.indexOf("</nav>", erpIdx);
if (start !== -1 && end !== -1) {
  ...
  if(nav.match(adminRegex)) {
    nav = nav.replace(adminRegex, "$1" + injection);
  } else {
    console.log("Admin regex did not match!");
  }
