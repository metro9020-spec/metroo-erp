...
const submitRegex = /document\.getElementById\("create-invoice-form"\)\.addEventListener\("submit",\s*async\s*\(e\)\s*=>\s*\{[\s\S]*?const newInv = state\.createInvoice\(payload\);\s*\n\s*\/\/ Release Lock\s*\n\s*try \{ await fetch[^}]+\} catch\(e\) \{\}\s*\n\s*if\(btn\) \{ btn\.disabled = false; btn\.textContent = "Save Invoice"; \}/;

const orig = `document.getElementById("create-invoice-form").addEventListener("submit", (e) => {
    e.preventDefault();
...
if (submitRegex.test(code)) {
    code = code.replace(submitRegex, orig);
    fs.writeFileSync('G:/sw m/erp/src/views/transactions.js', code);
    console.log('Reverted transactions.js successfully');
} else {
