...
stateJs = stateJs.replace(/3001\/api\/data\/" \+ activeId/g, "3001/api/data/${activeId}`");
...
mainJs = mainJs.replace(/3001\/api\/backup\/" \+ activeId/g, "3001/api/backup/${activeId}`");
fs.writeFileSync("src/main.js", mainJs);

console.log("Fixed hostnames string literal injection");
