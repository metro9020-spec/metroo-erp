...
for (let l of lines) {
  if (l.includes("erp-menu-bar") && l.includes("Masters") && l.includes("Transaction")) {
    try {
      const obj = JSON.parse(l);
      if (obj.content && obj.content.includes("erp-menu-bar") && obj.content.includes("Masters")) {
        const c = obj.content;
        ...
        if (start !== -1 && end !== -1) {
          ...
          console.log("Extracted REAL HTML via JSON parse!");
          break;
        }
      }
    } catch(e) {}
